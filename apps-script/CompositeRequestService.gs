...
function compositeVenueEquipmentRequestsEnabled_() { return typeof venueEquipmentRequestsEnabled_ === 'function' && venueEquipmentRequestsEnabled_(); }
...
function compositePositiveInteger_(value, field) { var number = Number(value); if (!isFinite(number) || Math.floor(number) !== number || number < 1) throw appError_('VALIDATION_ERROR', field + ' must be a positive integer.', false, { field: field }); return number; }
...
function compositeLine_(line, index) {
  ...
  return { referenceId: compositeNormalizeText_(line.referenceId || line.itemId, 'lines[' + index + '].referenceId', false, 100), referenceRevision: line.referenceRevision == null || line.referenceRevision === '' ? null : compositePositiveInteger_(line.referenceRevision, 'lines[' + index + '].referenceRevision'), label: compositeNormalizeText_(line.label || line.description || line.name, 'lines[' + index + '].label', true), quantity: compositePositive_(line.quantity, 'lines[' + index + '].quantity'), unit: compositeNormalizeText_(line.unit, 'lines[' + index + '].unit', true, 40).toLowerCase(), category: compositeNormalizeText_(line.category, 'lines[' + index + '].category', false, 80).toUpperCase(), notes: compositeNormalizeText_(line.notes, 'lines[' + index + '].notes', false) };
}
...
function compositeSection_(section, index, context) {
  ...
  var result = { type: type, label: label || HAU_COMPOSITE_SECTION_LABELS_[type], notes: notes, clientComponentId: compositeNormalizeText_(section.clientComponentId, 'clientComponentId', false, 120), lines: compositeConsolidateLines_(lines.map(compositeLine_)) };
  if (type === 'FOOD') result.food = foodDetails_(section.food, context && context.submittedAt, context && context.eventStartAt);
  if (type === 'MATERIALS') { result.materials = materialsDetails_(section.materials, result.lines, context && context.existingMaterials); result.lines = result.materials.lines; delete result.materials.lines; }
  if (type === 'VENUE_EQUIPMENT' && (section.venueEquipment || context && context.requireVenueEquipmentDetails)) { result.venueEquipment = venueEquipmentDetails_(section.venueEquipment, result.lines, context || {}); result.lines = result.venueEquipment.lines; delete result.venueEquipment.lines; }
  return result;
}
function compositeDraft_(command) {
  ...
  var eventStartAt = compositeNormalizeText_(command.eventStartAt || event.startAt, 'eventStartAt', false, 80), submittedAt = nowIso_();
  var eventEndAt = compositeNormalizeText_(command.eventEndAt || event.endAt, 'eventEndAt', false, 80);
  var sections = (Array.isArray(command.sections) ? command.sections : []).map(function(section, index) { return compositeSection_(section, index, { submittedAt: submittedAt, eventStartAt: eventStartAt, eventEndAt: eventEndAt, requireVenueEquipmentDetails: compositeVenueEquipmentRequestsEnabled_() }); }).filter(function(section) { return Boolean(section); });
  var seen = {};
  ...
  return { requesterName: requesterName, requesterEmail: requesterEmail, department: department, purpose: purpose, eventSeriesId: compositeNormalizeText_(command.eventSeriesId || event.seriesId, 'eventSeriesId', false, 120), eventId: eventId, eventName: compositeNormalizeText_(command.eventName || event.name, 'eventName', false), eventStartAt: eventStartAt, eventEndAt: eventEndAt, priority: compositeNormalizeText_(command.priority, 'priority', false, 40) || 'ROUTINE', sections: sections };
}
function compositePayload_(section) { var payload = { notes: section.notes, lines: section.lines.map(function(line) { var copy = Object.assign({}, line); delete copy.duplicateCount; return copy; }) }; if (section.food) payload.food = section.food; if (section.materials) payload.materials = section.materials; if (section.venueEquipment) payload.venueEquipment = section.venueEquipment; return safeJson_(payload); }
function compositeSectionAttentionFlags_(section) { return section.food ? foodAttentionFlags_(section.food) : section.materials ? materialsAttentionFlags_(section.materials) : section.venueEquipment && typeof venueEquipmentAttentionFlags_ === 'function' ? venueEquipmentAttentionFlags_(section.venueEquipment) : []; }
...
function compositeParentDto_(row, children, derivationChildren) {
  var derived = compositeDerive_(derivationChildren || children);
...
function compositeDerive_(children) {
  ...
  if (children.some(function(child) { return compositeParse_(child.Attention_Flags_JSON, []).some(function(flag) { return String(flag).indexOf('FOOD_') === 0; }); })) attentionFlags.push('FOOD_ATTENTION');
  if (children.some(function(child) { return compositeParse_(child.Attention_Flags_JSON, []).some(function(flag) { return String(flag).indexOf('MATERIALS_') === 0; }); })) attentionFlags.push('MATERIALS_ATTENTION');
  if (children.some(function(child) { return compositeParse_(child.Attention_Flags_JSON, []).some(function(flag) { return String(flag).indexOf('VENUE_EQUIPMENT_') === 0; }); })) attentionFlags.push('VENUE_EQUIPMENT_ATTENTION');
...
function compositeTransition_(child, action, reason) {
  ...
  var reopenedFlags = [];
  if (normalized === 'REOPEN' && String(child.Component_Type) === 'FOOD') reopenedFlags = foodAttentionFlags_(compositeParse_(child.Component_Payload_JSON, {}).food || {});
  if (normalized === 'REOPEN' && String(child.Component_Type) === 'MATERIALS') reopenedFlags = materialsAttentionFlags_(compositeParse_(child.Component_Payload_JSON, {}).materials || {});
  if (normalized === 'REOPEN' && String(child.Component_Type) === 'VENUE_EQUIPMENT' && compositeParse_(child.Component_Payload_JSON, {}).venueEquipment && typeof venueEquipmentAttentionFlags_ === 'function') reopenedFlags = venueEquipmentAttentionFlags_(compositeParse_(child.Component_Payload_JSON, {}).venueEquipment);
  return { status: next, attentionFlags: normalized === 'REOPEN' ? reopenedFlags : compositeParse_(child.Attention_Flags_JSON, []).filter(function(flag) { return flag !== 'NEEDS_INFORMATION'; }), reason: compositeNormalizeText_(reason, 'reason', false, 500) };
}
function compositeRequireMutation_(command) { return requireIdempotency_(command); }
function compositeRequireFoodRevision_(command, child) {
  if (child && String(child.Component_Type) === 'FOOD' && Number(command.expectedRevision) !== Number(child.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Food component changed; refresh before updating.', true);
}
function compositeRequireFoodRevisionMap_(command, children, statuses) {
  var expected = command.expectedRevisions || {};
  children.filter(function(child) { return String(child.Component_Type) === 'FOOD' && statuses.indexOf(String(child.Lifecycle_Status)) >= 0; }).forEach(function(child) { if (Number(expected[child.Component_ID]) !== Number(child.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Food component changed; refresh before updating.', true); });
}
function compositeRequireMaterialsRevision_(command, child) {
  if (child && String(child.Component_Type) === 'MATERIALS' && Number(command.expectedRevision) !== Number(child.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Materials component changed; refresh before updating.', true);
}
function compositeRequireMaterialsRevisionMap_(command, children, statuses) {
  var expected = command.expectedRevisions || {};
  children.filter(function(child) { return String(child.Component_Type) === 'MATERIALS' && statuses.indexOf(String(child.Lifecycle_Status)) >= 0; }).forEach(function(child) { if (Number(expected[child.Component_ID]) !== Number(child.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Materials component changed; refresh before updating.', true); });
}
function compositeRequireVenueEquipmentRevision_(command, child) {
  if (child && String(child.Component_Type) === 'VENUE_EQUIPMENT' && compositeParse_(child.Component_Payload_JSON, {}).venueEquipment && Number(command.expectedRevision) !== Number(child.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Venue and Equipment component changed; refresh before updating.', true);
}
function compositeRequireVenueEquipmentRevisionMap_(command, children, statuses) {
  var expected = command.expectedRevisions || {};
  children.filter(function(child) { return String(child.Component_Type) === 'VENUE_EQUIPMENT' && compositeParse_(child.Component_Payload_JSON, {}).venueEquipment && statuses.indexOf(String(child.Lifecycle_Status)) >= 0; }).forEach(function(child) { if (Number(expected[child.Component_ID]) !== Number(child.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Venue and Equipment component changed; refresh before updating.', true); });
}
...
function submitCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    ...
    if ((command.sections || []).some(function(section) { return String(section && section.type || '').trim().toUpperCase() === 'MATERIALS'; }) && !materialsRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Materials request specialization is not enabled.', false, { feature: 'HAU_MATERIALS_REQUESTS_ENABLED' });
    if ((command.sections || []).some(function(section) { return String(section && section.type || '').trim().toUpperCase() === 'VENUE_EQUIPMENT' && section.venueEquipment; }) && !compositeVenueEquipmentRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Venue and Equipment request specialization is not enabled.', false, { feature: 'HAU_VENUE_EQUIPMENT_REQUESTS_ENABLED' });
    var user = resolveRequesterUser_(), draft = compositeDraft_(command);
    if (draft.sections.some(function(section) { return section.type === 'FOOD'; }) && !foodRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Food request specialization is not enabled.', false, { feature: 'HAU_FOOD_REQUESTS_ENABLED' });
    if (draft.sections.some(function(section) { return section.type === 'MATERIALS'; }) && !materialsRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Materials request specialization is not enabled.', false, { feature: 'HAU_MATERIALS_REQUESTS_ENABLED' });
    var requestId = allocateId_('LREQ'), componentIds = draft.sections.map(function() { return allocateId_('CMP'); }), now = nowIso_(), rows = compositeRowsFromDraft_(draft, requestId, componentIds, user, key, now);
...
function getCompositeRequest_(command) {
  return withRequestReadCache_(function() {
    ...
    if (String(user.User_ID || 'PUBLIC') === 'PUBLIC') throw appError_('FORBIDDEN', 'Direct composite request reads require an authenticated requester or internal user.', false);
    if (String(parent.Created_By || '') !== String(user.User_ID || '') && !internal) throw appError_('FORBIDDEN', 'This composite request is outside your requester scope.', false);
    var allChildren = compositeChildren_(rows), children = allChildren;
    if (internal && typeof authorizationCan_ === 'function' && typeof HAU_CAPABILITIES_ !== 'undefined') children = children.filter(function(child) { return authorizationCan_(user, HAU_CAPABILITIES_.VIEW_REQUEST, { committeeIds: [child.Owner_Committee_ID] }, { allowUnresolvedScope: false }); });
    if (internal && !children.length) throw appError_('FORBIDDEN', 'This composite request is outside your committee scope.', false);
    return { request: compositeParentDto_(parent, children, allChildren) };
  });
}
...
function transitionCompositeComponent_(command, correlationId) {
  return withScriptLock_(function() {
    ...
    var parent = compositeFindParent_(rows), user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }); compositeRequireFoodRevision_(command, child); compositeRequireMaterialsRevision_(command, child); compositeRequireVenueEquipmentRevision_(command, child); foodAssertTransition_(child, command.action); materialsAssertTransition_(child, command.action); if (typeof venueEquipmentAssertTransition_ === 'function') venueEquipmentAssertTransition_(child, command.action); var next = compositeTransition_(child, command.action, command.reason), now = nowIso_();
...
function cancelCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), children = compositeChildren_(rows), user = requirePermission_('Can_Review', { committeeIds: children.map(function(child) { return child.Owner_Committee_ID; }) }), now = nowIso_(); if (!parent) throw appError_('REQUEST_NOT_FOUND', 'Composite request was not found.', false); compositeRequireFoodRevisionMap_(command, children, ['DRAFT', 'FOR_REVIEW', 'ACCEPTED', 'IN_PROGRESS', 'PARTIALLY_FULFILLED', 'READY_FOR_HANDOFF']); compositeRequireMaterialsRevisionMap_(command, children, ['DRAFT', 'FOR_REVIEW', 'ACCEPTED', 'IN_PROGRESS', 'PARTIALLY_FULFILLED', 'READY_FOR_HANDOFF']); compositeRequireVenueEquipmentRevisionMap_(command, children, ['DRAFT', 'FOR_REVIEW', 'ACCEPTED', 'IN_PROGRESS', 'PARTIALLY_FULFILLED', 'READY_FOR_HANDOFF']);
...
function reopenCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), children = compositeChildren_(rows), user = requirePermission_('Can_Review', { committeeIds: children.map(function(child) { return child.Owner_Committee_ID; }) }), now = nowIso_(); if (!parent) throw appError_('REQUEST_NOT_FOUND', 'Composite request was not found.', false); compositeRequireFoodRevisionMap_(command, children, ['REJECTED', 'CANCELLED']); compositeRequireMaterialsRevisionMap_(command, children, ['REJECTED', 'CANCELLED']); compositeRequireVenueEquipmentRevisionMap_(command, children, ['REJECTED', 'CANCELLED']); if (['REJECTED', 'CANCELLED'].indexOf(String(parent.Lifecycle_Status)) < 0) throw appError_('INVALID_TRANSITION', 'Only rejected or cancelled composite requests may be reopened.', false);
    children.forEach(function(child) { if (['REJECTED', 'CANCELLED'].indexOf(String(child.Lifecycle_Status)) >= 0) { var reopened = compositeTransition_(child, 'REOPEN', command.reason); updateObject_(HAU_SHEETS.COMPOSITE_REQUESTS, child._row, { Lifecycle_Status: reopened.status, Attention_Flags_JSON: safeJson_(reopened.attentionFlags), Revision: Number(child.Revision || 1) + 1, Updated_At: now }); history_('COMPOSITE_COMPONENT', child.Component_ID, child.Lifecycle_Status, 'FOR_REVIEW', user, command.reason || 'Composite request reopened', { requestId: parent.Request_ID, idempotencyKey: key }); } });
...
function amendCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), child = compositeChildren_(rows).find(function(row) { return String(row.Component_ID) === String(command.componentId); }); if (!parent || !child) throw appError_('COMPONENT_NOT_FOUND', 'Composite section was not found.', false); var user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }), now = nowIso_(); compositeRequireFoodRevision_(command, child); compositeRequireMaterialsRevision_(command, child); compositeRequireVenueEquipmentRevision_(command, child); if (HAU_COMPOSITE_TERMINAL_STATUSES_.indexOf(String(child.Lifecycle_Status)) >= 0) throw appError_('AMENDMENT_NOT_ALLOWED', 'Completed, rejected, or cancelled sections cannot be amended directly.', false); var currentPayload = compositeParse_(child.Component_Payload_JSON, { lines: [] }), sectionPatch = command.section || {}, materialsDraft = currentPayload.materials ? Object.assign({}, currentPayload.materials, sectionPatch.materials || {}) : sectionPatch.materials, venueEquipmentDraft = currentPayload.venueEquipment ? Object.assign({}, currentPayload.venueEquipment, sectionPatch.venueEquipment || {}) : sectionPatch.venueEquipment, section = compositeSection_(Object.assign({}, sectionPatch, { type: child.Component_Type, label: sectionPatch.label || child.Label, lines: sectionPatch.lines || currentPayload.lines, food: sectionPatch.food || currentPayload.food, materials: materialsDraft, venueEquipment: venueEquipmentDraft }), 0, { submittedAt: now, eventStartAt: parent.Event_Start_At, eventEndAt: parent.Event_End_At, existingMaterials: currentPayload.materials || null, existingVenueEquipment: currentPayload.venueEquipment || null, requireVenueEquipmentDetails: Boolean(currentPayload.venueEquipment) }); if (!section) throw appError_('VALIDATION_ERROR', 'An amended section cannot be blank.', false);
    var amendedFlags = section.food || section.materials || section.venueEquipment ? compositeSectionAttentionFlags_(section) : compositeParse_(child.Attention_Flags_JSON, []); if (amendedFlags.indexOf('AMENDED') < 0) amendedFlags.push('AMENDED');
    updateObject_(HAU_SHEETS.COMPOSITE_REQUESTS, child._row, { Label: section.label, Lifecycle_Status: 'FOR_REVIEW', Owner_Committee_ID: section.venueEquipment ? section.venueEquipment.ownerCommitteeId : child.Owner_Committee_ID, Owner_User_ID: section.venueEquipment ? section.venueEquipment.ownerUserId : child.Owner_User_ID, Attention_Flags_JSON: safeJson_(amendedFlags), Component_Payload_JSON: compositePayload_(section), Relationship_Version: Number(child.Relationship_Version || 1) + 1, Revision: Number(child.Revision || 1) + 1, Updated_At: now, Notes: section.notes });
...
function addCompositeSection_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), now = nowIso_(); if (!parent) throw appError_('REQUEST_NOT_FOUND', 'Composite request was not found.', false);
    var rawSectionType = compositeSectionType_((command.section || {}).type);
    if (rawSectionType === 'MATERIALS' && !materialsRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Materials request specialization is not enabled.', false, { feature: 'HAU_MATERIALS_REQUESTS_ENABLED' });
    if (rawSectionType === 'VENUE_EQUIPMENT' && command.section && command.section.venueEquipment && !compositeVenueEquipmentRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Venue and Equipment request specialization is not enabled.', false, { feature: 'HAU_VENUE_EQUIPMENT_REQUESTS_ENABLED' });
    var section = compositeSection_(command.section || {}, 0, { submittedAt: now, eventStartAt: parent.Event_Start_At, eventEndAt: parent.Event_End_At, requireVenueEquipmentDetails: rawSectionType === 'VENUE_EQUIPMENT' && compositeVenueEquipmentRequestsEnabled_() });
    var committeeId = section.venueEquipment ? section.venueEquipment.ownerCommitteeId : HAU_COMPOSITE_COMMITTEES_[section.type], user = requirePermission_('Can_Review', { committeeIds: [committeeId] });
    if (section.type === 'FOOD' && !foodRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Food request specialization is not enabled.', false, { feature: 'HAU_FOOD_REQUESTS_ENABLED' });
    if (section.type === 'MATERIALS' && !materialsRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Materials request specialization is not enabled.', false, { feature: 'HAU_MATERIALS_REQUESTS_ENABLED' });
    if (['FOOD', 'MATERIALS', 'VENUE_EQUIPMENT'].indexOf(section.type) >= 0 && Number(command.expectedParentRevision) !== Number(parent.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Composite request changed; refresh before adding ' + HAU_COMPOSITE_SECTION_LABELS_[section.type] + '.', true);
    if (compositeChildren_(rows).some(function(row) { return String(row.Component_Type) === section.type; })) throw appError_('DUPLICATE_COMPONENT', 'That composite section already exists.', false); if (['COMPLETED', 'CANCELLED'].indexOf(String(parent.Lifecycle_Status)) >= 0) throw appError_('AMENDMENT_NOT_ALLOWED', 'A completed or cancelled composite request cannot receive a new section.', false);
    var componentId = allocateId_('CMP'); appendObject_(HAU_SHEETS.COMPOSITE_REQUESTS, { Record_ID: componentId, Record_Type: 'COMPONENT', Request_ID: parent.Request_ID, Component_ID: componentId, Relationship_Type: 'COMPOSITE_CHILD', Relationship_Version: 1, Component_Type: section.type, Label: section.label, Requester_Name: '', Requester_Email: '', Department: '', Event_Series_ID: '', Event_ID: parent.Event_ID, Event_Name: '', Event_Start_At: '', Event_End_At: '', Priority: parent.Priority, Purpose: '', Component_Payload_JSON: compositePayload_(section), Lifecycle_Status: 'FOR_REVIEW', Owner_Committee_ID: committeeId, Owner_User_ID: section.venueEquipment ? section.venueEquipment.ownerUserId : '', Due_At: parent.Due_At, Attention_Flags_JSON: safeJson_(compositeSectionAttentionFlags_(section)), Progress_JSON: safeJson_({ lineCount: section.lines.length, duplicateCount: section.lines.reduce(function(sum, line) { return sum + Number(line.duplicateCount || 0); }, 0) }), Revision: 1, Created_At: now, Updated_At: now, Created_By: user.User_ID || 'SYSTEM', Client_Request_ID: '', Client_Component_ID: compositeNormalizeText_(command.section.clientComponentId, 'clientComponentId', false, 120), Idempotency_Key: key, Archived_At: '', Notes: section.notes });
...
function assignCompositeComponent_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var componentId = requireText_(command.componentId, 'componentId'), rows = compositeRowsForRequest_(command.requestId || ''), child = compositeChildren_(rows).find(function(row) { return String(row.Component_ID) === String(componentId); }); if (!child) throw appError_('COMPONENT_NOT_FOUND', 'Composite section was not found.', false); var committeeId = requireText_(command.committeeId || child.Owner_Committee_ID, 'committeeId'), user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }), assignee = compositeNormalizeText_(command.userId, 'userId', false, 120), now = nowIso_(); compositeRequireFoodRevision_(command, child); compositeRequireMaterialsRevision_(command, child); compositeRequireVenueEquipmentRevision_(command, child); if (committeeId !== child.Owner_Committee_ID) throw appError_('ASSIGNMENT_SCOPE_INVALID', 'A composite section must remain with its canonical committee.', false); if (assignee) { var target = findOne_(HAU_SHEETS.USERS, 'User_ID', assignee); if (!target || !accessRowEligible_(target)) throw appError_('ASSIGNEE_INVALID', 'The selected assignee is not active.', false); }
