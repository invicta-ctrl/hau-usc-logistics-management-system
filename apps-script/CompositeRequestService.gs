...
function compositeDraft_(command) {
  ...
  var sections = (Array.isArray(command.sections) ? command.sections : []).map(function(section, index) { return compositeSection_(section, index, { submittedAt: submittedAt, eventStartAt: eventStartAt }); }).filter(function(section) { return Boolean(section); });
  var seen = {};
  ...
  return { requesterName: requesterName, requesterEmail: requesterEmail, department: department, purpose: purpose, eventSeriesId: compositeNormalizeText_(command.eventSeriesId || event.seriesId, 'eventSeriesId', false, 120), eventId: eventId, eventName: compositeNormalizeText_(command.eventName || event.name, 'eventName', false), eventStartAt: eventStartAt, eventEndAt: compositeNormalizeText_(command.eventEndAt || event.endAt, 'eventEndAt', false, 80), priority: compositeNormalizeText_(command.priority, 'priority', false, 40) || 'ROUTINE', sections: sections };
}
function compositePayload_(section) { var payload = { notes: section.notes, lines: section.lines.map(function(line) { var copy = Object.assign({}, line); delete copy.duplicateCount; return copy; }) }; if (section.food) payload.food = section.food; if (section.materials) payload.materials = section.materials; return safeJson_(payload); }
function compositeSectionAttentionFlags_(section) { return section.food ? foodAttentionFlags_(section.food) : section.materials ? materialsAttentionFlags_(section.materials) : []; }
...
function transitionCompositeComponent_(command, correlationId) {
  return withScriptLock_(function() {
    ...
    var parent = compositeFindParent_(rows), user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }); compositeRequireFoodRevision_(command, child); compositeRequireMaterialsRevision_(command, child); foodAssertTransition_(child, command.action); materialsAssertTransition_(child, command.action); var next = compositeTransition_(child, command.action, command.reason), now = nowIso_();
...
function reopenCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), children = compositeChildren_(rows), user = requirePermission_('Can_Review', { committeeIds: children.map(function(child) { return child.Owner_Committee_ID; }) }), now = nowIso_(); if (!parent) throw appError_('REQUEST_NOT_FOUND', 'Composite request was not found.', false); compositeRequireFoodRevisionMap_(command, children, ['REJECTED', 'CANCELLED']); compositeRequireMaterialsRevisionMap_(command, children, ['REJECTED', 'CANCELLED']); if (['REJECTED', 'CANCELLED'].indexOf(String(parent.Lifecycle_Status)) < 0) throw appError_('INVALID_TRANSITION', 'Only rejected or cancelled composite requests may be reopened.', false);
...
function amendCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), child = compositeChildren_(rows).find(function(row) { return String(row.Component_ID) === String(command.componentId); }); if (!parent || !child) throw appError_('COMPONENT_NOT_FOUND', 'Composite section was not found.', false); var user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }), now = nowIso_(); compositeRequireFoodRevision_(command, child); compositeRequireMaterialsRevision_(command, child); if (HAU_COMPOSITE_TERMINAL_STATUSES_.indexOf(String(child.Lifecycle_Status)) >= 0) throw appError_('AMENDMENT_NOT_ALLOWED', 'Completed, rejected, or cancelled sections cannot be amended directly.', false); var currentPayload = compositeParse_(child.Component_Payload_JSON, { lines: [] }), sectionPatch = command.section || {}, materialsDraft = currentPayload.materials ? Object.assign({}, currentPayload.materials, sectionPatch.materials || {}) : sectionPatch.materials, section = compositeSection_(Object.assign({}, sectionPatch, { type: child.Component_Type, label: sectionPatch.label || child.Label, lines: sectionPatch.lines || currentPayload.lines, food: sectionPatch.food || currentPayload.food, materials: materialsDraft }), 0, { submittedAt: child.Created_At, eventStartAt: parent.Event_Start_At, existingMaterials: currentPayload.materials || null }); if (!section) throw appError_('VALIDATION_ERROR', 'An amended section cannot be blank.', false);
    var amendedFlags = section.food || section.materials ? compositeSectionAttentionFlags_(section) : compositeParse_(child.Attention_Flags_JSON, []); if (amendedFlags.indexOf('AMENDED') < 0) amendedFlags.push('AMENDED');
    updateObject_(HAU_SHEETS.COMPOSITE_REQUESTS, child._row, { Label: section.label, Lifecycle_Status: 'FOR_REVIEW', Attention_Flags_JSON: safeJson_(amendedFlags), Component_Payload_JSON: compositePayload_(section), Relationship_Version: Number(child.Relationship_Version || 1) + 1, Revision: Number(child.Revision || 1) + 1, Updated_At: now, Notes: section.notes });
...
function addCompositeSection_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), now = nowIso_(); if (!parent) throw appError_('REQUEST_NOT_FOUND', 'Composite request was not found.', false); var rawSectionType = compositeSectionType_((command.section || {}).type); if (rawSectionType === 'MATERIALS' && !materialsRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Materials request specialization is not enabled.', false, { feature: 'HAU_MATERIALS_REQUESTS_ENABLED' }); var section = compositeSection_(command.section || {}, 0, { submittedAt: now, eventStartAt: parent.Event_Start_At }), committeeId = HAU_COMPOSITE_COMMITTEES_[section.type], user = requirePermission_('Can_Review', { committeeIds: [committeeId] }); if (section.type === 'FOOD' && !foodRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Food request specialization is not enabled.', false, { feature: 'HAU_FOOD_REQUESTS_ENABLED' }); if (section.type === 'MATERIALS' && !materialsRequestsEnabled_()) throw appError_('FEATURE_DISABLED', 'Materials request specialization is not enabled.', false, { feature: 'HAU_MATERIALS_REQUESTS_ENABLED' }); if (section.type === 'FOOD' && Number(command.expectedParentRevision) !== Number(parent.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Composite request changed; refresh before adding Food.', true); if (section.type === 'MATERIALS' && Number(command.expectedParentRevision) !== Number(parent.Revision || 1)) throw appError_('REVISION_CONFLICT', 'Composite request changed; refresh before adding Materials.', true); if (compositeChildren_(rows).some(function(row) { return String(row.Component_Type) === section.type; })) throw appError_('DUPLICATE_COMPONENT', 'That composite section already exists.', false); if (['COMPLETED', 'CANCELLED'].indexOf(String(parent.Lifecycle_Status)) >= 0) throw appError_('AMENDMENT_NOT_ALLOWED', 'A completed or cancelled composite request cannot receive a new section.', false);
    var componentId = allocateId_('CMP'); appendObject_(HAU_SHEETS.COMPOSITE_REQUESTS, { Record_ID: componentId, Record_Type: 'COMPONENT', Request_ID: parent.Request_ID, Component_ID: componentId, Relationship_Type: 'COMPOSITE_CHILD', Relationship_Version: 1, Component_Type: section.type, Label: section.label, Requester_Name: '', Requester_Email: '', Department: '', Event_Series_ID: '', Event_ID: parent.Event_ID, Event_Name: '', Event_Start_At: '', Event_End_At: '', Priority: parent.Priority, Purpose: '', Component_Payload_JSON: compositePayload_(section), Lifecycle_Status: 'FOR_REVIEW', Owner_Committee_ID: committeeId, Owner_User_ID: '', Due_At: parent.Due_At, Attention_Flags_JSON: safeJson_(compositeSectionAttentionFlags_(section)), Progress_JSON: safeJson_({ lineCount: section.lines.length, duplicateCount: section.lines.reduce(function(sum, line) { return sum + Number(line.duplicateCount || 0); }, 0) }), Revision: 1, Created_At: now, Updated_At: now, Created_By: user.User_ID || 'SYSTEM', Client_Request_ID: '', Client_Component_ID: compositeNormalizeText_(command.section.clientComponentId, 'clientComponentId', false, 120), Idempotency_Key: key, Archived_At: '', Notes: section.notes });
