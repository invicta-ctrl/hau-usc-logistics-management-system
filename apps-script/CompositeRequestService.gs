...
function compositePayload_(section) { var payload = { notes: section.notes, lines: section.lines.map(function(line) { var copy = Object.assign({}, line); delete copy.duplicateCount; return copy; }) }; if (section.food) payload.food = section.food; return safeJson_(payload); }
...
function transitionCompositeComponent_(command, correlationId) {
  return withScriptLock_(function() {
    ...
    var parent = compositeFindParent_(rows), user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }); compositeRequireFoodRevision_(command, child); foodAssertTransition_(child, command.action); var next = compositeTransition_(child, command.action, command.reason), now = nowIso_();
...
function reopenCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), children = compositeChildren_(rows), user = requirePermission_('Can_Review', { committeeIds: children.map(function(child) { return child.Owner_Committee_ID; }) }), now = nowIso_(); if (!parent) throw appError_('REQUEST_NOT_FOUND', 'Composite request was not found.', false); compositeRequireFoodRevisionMap_(command, children, ['REJECTED', 'CANCELLED']); if (['REJECTED', 'CANCELLED'].indexOf(String(parent.Lifecycle_Status)) < 0) throw appError_('INVALID_TRANSITION', 'Only rejected or cancelled composite requests may be reopened.', false);
...
function amendCompositeRequest_(command, correlationId) {
  return withScriptLock_(function() {
    var key = requireIdempotency_(command), replay = idempotencyReplay_(key); if (replay) return Object.assign({ idempotentReplay: true }, replay);
    var rows = compositeRowsForRequest_(requireText_(command.requestId, 'requestId')), parent = compositeFindParent_(rows), child = compositeChildren_(rows).find(function(row) { return String(row.Component_ID) === String(command.componentId); }); if (!parent || !child) throw appError_('COMPONENT_NOT_FOUND', 'Composite section was not found.', false); var user = requirePermission_('Can_Review', { committeeIds: [child.Owner_Committee_ID] }), now = nowIso_(); compositeRequireFoodRevision_(command, child); if (HAU_COMPOSITE_TERMINAL_STATUSES_.indexOf(String(child.Lifecycle_Status)) >= 0) throw appError_('AMENDMENT_NOT_ALLOWED', 'Completed, rejected, or cancelled sections cannot be amended directly.', false); var currentPayload = compositeParse_(child.Component_Payload_JSON, { lines: [] }), section = compositeSection_(Object.assign({}, command.section || {}, { type: child.Component_Type, label: (command.section || {}).label || child.Label, lines: (command.section || {}).lines || currentPayload.lines, food: (command.section || {}).food || currentPayload.food }), 0, { submittedAt: child.Created_At, eventStartAt: parent.Event_Start_At }); if (!section) throw appError_('VALIDATION_ERROR', 'An amended section cannot be blank.', false);
    var amendedFlags = section.food ? foodAttentionFlags_(section.food) : compositeParse_(child.Attention_Flags_JSON, []); if (amendedFlags.indexOf('AMENDED') < 0) amendedFlags.push('AMENDED');
