...
function positiveNumber_(value, field) { var n = Number(value); if (!isFinite(n) || n <= 0) throw appError_('VALIDATION_ERROR', field + ' must be greater than zero.', false, { field: field }); return n; }
function nonNegativeNumber_(value, field) { var n = Number(value || 0); if (!isFinite(n) || n < 0) throw appError_('VALIDATION_ERROR', field + ' cannot be negative.', false, { field: field }); return n; }
...
function guardApi_(operationName, command, fn) {
  ...
  try { var result = fn(correlationId); return Object.assign({ ok: true, correlationId: correlationId }, result || {}); }
  catch (error) { try { logError_(operationName, command, error, correlationId); } catch (ignored) {} return publicError_(error, correlationId); }
}
