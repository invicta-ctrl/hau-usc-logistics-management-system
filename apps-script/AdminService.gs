...
function permissionInput_(command,name){var aliases={Can_Review:'review',Can_Release:'release',Can_Receive:'receive',Can_Admin:'admin',Can_Manage_Catalog:'manageCatalog'},alias=aliases[name],nested=command.permissions&&typeof command.permissions==='object'?command.permissions:{};if(Object.prototype.hasOwnProperty.call(command,name))return{present:true,value:command[name]};if(alias&&Object.prototype.hasOwnProperty.call(command,alias))return{present:true,value:command[alias]};if(alias&&Object.prototype.hasOwnProperty.call(nested,alias))return{present:true,value:nested[alias]};return{present:false,value:false};}
...
function contentDto_(row){var body=null;try{body=JSON.parse(row.Body_JSON||'null');}catch(ignored){}return{revisionId:row.Content_Revision_ID,key:row.Content_Key,type:row.Content_Type,title:row.Title,body:body,status:row.Status,revision:Number(row.Revision_Number||0),publishedAt:row.Published_At||'',createdAt:row.Created_At||'',updatedAt:row.Updated_At||''};}
...
function nextContentRevision_(key){return adminRows_(HAU_SHEETS.CONTENT).filter(function(row){return String(row.Content_Key)===key;}).reduce(function(max,row){return Math.max(max,Number(row.Revision_Number||0));},0)+1;}
function saveContentRevision_(command,correlationId){return withScriptLock_(function(){var user=requirePermission_('Can_Admin'),key=requireIdempotency_(command),replay=idempotencyReplay_(key,user);if(replay)return Object.assign({idempotentReplay:true},replay);var contentKey=sanitizeSystemToken_(adminText_(command.contentKey||command.key,'contentKey',80)),type=String(command.contentType||command.type||'JSON').toUpperCase();if(HAU_CONTENT_TYPES_.indexOf(type)<0)throw appError_('VALIDATION_ERROR','Choose a supported content type.',false,{field:'contentType'});var now=nowIso_(),row={Content_Revision_ID:allocateId_('CNT'),Content_Key:contentKey,Content_Type:type,Title:adminText_(command.title,'title',180),Body_JSON:contentBodyJson_(command.bodyJson==null?command.body:command.bodyJson),Status:'DRAFT',Revision_Number:nextContentRevision_(contentKey),Published_At:'',Published_By:'',Created_At:now,Created_By:user.User_ID,Updated_At:now,Updated_By:user.User_ID,Supersedes_Revision_ID:String(command.supersedesRevisionId||''),Reverted_From_Revision_ID:'',Notes:String(command.notes||'').slice(0,1000)};appendObject_(HAU_SHEETS.CONTENT,row);audit_('SAVE_CONTENT_DRAFT','CONTENT',row.Content_Revision_ID,user,correlationId,{after:{key:row.Content_Key,type:row.Content_Type,title:row.Title,revision:row.Revision_Number,status:row.Status}});return recordIdempotency_(key,{content:contentDto_(row),contentRevisionId:row.Content_Revision_ID,entityType:'CONTENT',entityId:row.Content_Revision_ID},user,correlationId);});}
function contentRevision_(command){var rows=adminRows_(HAU_SHEETS.CONTENT),id=String(command.contentRevisionId||command.revisionId||'');if(id)return rows.find(function(row){return String(row.Content_Revision_ID)===id;})||null;var key=sanitizeSystemToken_(command.contentKey||command.key||'');return rows.filter(function(row){return String(row.Content_Key)===key&&String(row.Status)==='DRAFT';}).sort(function(a,b){return Number(b.Revision_Number||0)-Number(a.Revision_Number||0);})[0]||null;}
function publishContentRevision_(command,correlationId){return withScriptLock_(function(){var user=requirePermission_('Can_Admin'),key=requireIdempotency_(command),replay=idempotencyReplay_(key,user);if(replay)return Object.assign({idempotentReplay:true},replay);var target=contentRevision_(command);if(!target)throw appError_('CONTENT_REVISION_NOT_FOUND','The content revision was not found.',false);adminRows_(HAU_SHEETS.CONTENT).filter(function(row){return String(row.Content_Key)===String(target.Content_Key)&&String(row.Status)==='PUBLISHED'&&String(row.Content_Revision_ID)!==String(target.Content_Revision_ID);}).forEach(function(row){updateObject_(HAU_SHEETS.CONTENT,row._row,{Status:'SUPERSEDED',Updated_At:nowIso_(),Updated_By:user.User_ID});});var patch={Status:'PUBLISHED',Published_At:nowIso_(),Published_By:user.User_ID,Updated_At:nowIso_(),Updated_By:user.User_ID};updateObject_(HAU_SHEETS.CONTENT,target._row,patch);var after=Object.assign({},target,patch);history_('CONTENT',target.Content_Revision_ID,target.Status,'PUBLISHED',user,command.reason||'Content published',{idempotencyKey:key});audit_('PUBLISH_CONTENT','CONTENT',target.Content_Revision_ID,user,correlationId,{after:{key:target.Content_Key,revision:target.Revision_Number,status:'PUBLISHED'}});return recordIdempotency_(key,{content:contentDto_(after),contentRevisionId:target.Content_Revision_ID,entityType:'CONTENT',entityId:target.Content_Revision_ID},user,correlationId);});}
function revertContentRevision_(command,correlationId){return withScriptLock_(function(){var user=requirePermission_('Can_Admin'),key=requireIdempotency_(command),replay=idempotencyReplay_(key,user);if(replay)return Object.assign({idempotentReplay:true},replay);var rows=adminRows_(HAU_SHEETS.CONTENT),contentKey=sanitizeSystemToken_(requireText_(command.contentKey||command.key,'contentKey')),current=rows.filter(function(row){return String(row.Content_Key)===contentKey&&String(row.Status)==='PUBLISHED';})[0],targetId=String(command.targetRevisionId||command.contentRevisionId||command.revisionId||''),target=targetId?rows.find(function(row){return String(row.Content_Revision_ID)===targetId&&String(row.Content_Key)===contentKey;}):rows.filter(function(row){return String(row.Content_Key)===contentKey&&(!current||String(row.Content_Revision_ID)!==String(current.Content_Revision_ID));}).sort(function(a,b){return Number(b.Revision_Number||0)-Number(a.Revision_Number||0);})[0];if(!target)throw appError_('CONTENT_REVISION_NOT_FOUND','No earlier content revision is available to revert.',false);if(current)updateObject_(HAU_SHEETS.CONTENT,current._row,{Status:'SUPERSEDED',Updated_At:nowIso_(),Updated_By:user.User_ID});var now=nowIso_(),row={Content_Revision_ID:allocateId_('CNT'),Content_Key:target.Content_Key,Content_Type:target.Content_Type,Title:target.Title,Body_JSON:target.Body_JSON,Status:'PUBLISHED',Revision_Number:nextContentRevision_(contentKey),Published_At:now,Published_By:user.User_ID,Created_At:now,Created_By:user.User_ID,Updated_At:now,Updated_By:user.User_ID,Supersedes_Revision_ID:current&&current.Content_Revision_ID||'',Reverted_From_Revision_ID:target.Content_Revision_ID,Notes:String(command.reason||'Published revision reverted').slice(0,1000)};appendObject_(HAU_SHEETS.CONTENT,row);history_('CONTENT',row.Content_Revision_ID,'','PUBLISHED',user,command.reason||'Content reverted',{idempotencyKey:key,metadata:{revertedFrom:target.Content_Revision_ID}});audit_('REVERT_CONTENT','CONTENT',row.Content_Revision_ID,user,correlationId,{after:{key:row.Content_Key,revision:row.Revision_Number,revertedFrom:target.Content_Revision_ID}});return recordIdempotency_(key,{content:contentDto_(row),contentRevisionId:row.Content_Revision_ID,entityType:'CONTENT',entityId:row.Content_Revision_ID},user,correlationId);});}
