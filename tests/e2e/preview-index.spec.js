...
function installVersion(page, playground) {
  return page.route(VERSION, (route) => route.fulfill({
    status: 200,
...
test('fails closed on every spoofed version signal regardless of hash, query, or storage', async ({ page }) => {
...
test('opens an auth route through the real session check without bypassing the workspace', async ({ page }) => {
  await installVersion(page, true);
  await installEmptyFeed(page);
  ...
  await page.goto('/#/__preview/index');
  await page.locator('[data-preview-route="inventory"] [data-action="open"]').click();
...
test('shows a labeled, sanitized, read-only surface preview with no additional API traffic', async ({ page }) => {
  await installVersion(page, true);
  await installEmptyFeed(page);
...
test('honors reduced motion on the preview index surface', async ({ page }) => {
  ...
  const duration = await page.locator('[data-preview-index]').evaluate((element) => getComputedStyle(element).animationDuration);
