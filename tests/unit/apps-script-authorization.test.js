...
describe('Apps Script canonical authorization contract', () => {
  ...
  it('enforces committee scope, Director oversight, inactive access, and capability separation', () => {
    ...
    expect(
      context.authorizationCan_(head, context.HAU_CAPABILITIES_.REQUEST_REVIEW, { committeeId: 'COM_FOOD' }),
    ).toBe(true);
    expect(
      context.authorizationCan_(head, context.HAU_CAPABILITIES_.REQUEST_REVIEW, {
        committeeId: 'COM_MATERIALS',
      }),
    ).toBe(false);
    expect(
      context.authorizationCan_(head, context.HAU_CAPABILITIES_.FULFILL_RELEASE, { committeeId: 'COM_FOOD' }),
    ).toBe(false);
    expect(
      context.authorizationCan_(director, context.HAU_CAPABILITIES_.VIEW_AUDIT, {
        committeeId: 'COM_MATERIALS',
      }),
    ).toBe(true);
    ...
    expect(
      context.authorizationDecision_(
        { User_ID: 'SYNTHETIC-INACTIVE', Role: 'DOL_STAFF', Committee: 'Food', Active: false },
        context.HAU_CAPABILITIES_.REQUEST_REVIEW,
        { committeeId: 'COM_FOOD' },
      ).reason,
    ).toBe('INACTIVE_USER');
    expect(
      context.authorizationDecision_(
        { User_ID: 'SYNTHETIC-UNKNOWN', Role: 'UNKNOWN_ROLE', Active: true },
        context.HAU_CAPABILITIES_.VIEW_REQUEST,
        {},
      ).reason,
    ).toBe('UNKNOWN_ROLE');
  });
  ...
  it('requires explicit mapping reconciliation and returns safe denial details', () => {
    ...
    const user = {
      User_ID: 'SYNTHETIC-HEAD',
      Role: 'COMMITTEE_HEAD',
      Committee: 'Unrecognized Committee',
      Active: true,
    };
    const decision = context.authorizationDecision_(user, context.HAU_CAPABILITIES_.REQUEST_REVIEW, {
      committeeId: 'COM_FOOD',
    });
    expect(decision.reason).toBe('MAPPING_RECONCILIATION_REQUIRED');
    expect(() =>
      context.authorizationRequire_(user, context.HAU_CAPABILITIES_.REQUEST_REVIEW, {
        committeeId: 'COM_FOOD',
      }),
    ).toThrow(
  ...
  it('fails closed when the login identity has multiple active access rows', () => {
    ...
    vm.runInContext(readFileSync(resolve(appScriptRoot, 'Auth.gs'), 'utf8'), context, {
      filename: 'Auth.gs',
    });
    context.Session = { getActiveUser: () => ({ getEmail: () => 'same@example.invalid' }) };
    expect(() => context.resolveCurrentUser_()).toThrow(
      expect.objectContaining({ code: 'AMBIGUOUS_IDENTITY' }),
    );
  });
  ...
  it('keeps audit visibility separate from operational review capability', () => {
    ...
    vm.runInContext(readFileSync(resolve(appScriptRoot, 'AuditService.gs'), 'utf8'), context, {
    ...
    expect(
      context.api_getAuditTimeline({ entityType: 'REQUEST', entityId: 'SYNTHETIC-ENTITY' }),
    ).toMatchObject({ ok: true, timeline: [] });
  });
  ...
  it('gates the legacy bootstrap endpoint and central authorization on the v2 rollout flag', () => {
    ...
    vm.runInContext(readFileSync(resolve(appScriptRoot, 'Router.gs'), 'utf8'), legacy, {
      filename: 'Router.gs',
    });
    ...
    vm.runInContext(readFileSync(resolve(appScriptRoot, 'Router.gs'), 'utf8'), canonical, {
      filename: 'Router.gs',
    });
    expect(canonical.api_getBootstrapData({ requestOnly: false })).toMatchObject({
      ok: false,
      code: 'LEGACY_BOOTSTRAP_DISABLED',
    });
  });
  ...
  it('resolves committee scope through linked request, delivery, canvass, and event-item records', () => {
    ...
    const records = [
      ...
      [
        context.HAU_SHEETS.REQUESTS,
        {
          Request_ID: 'SYNTHETIC-REQUEST',
          Assigned_Committee: 'Food Committee',
          Event_ID: 'SYNTHETIC-EVENT',
        },
      ],
      [
        context.HAU_SHEETS.REQUEST_LINES,
        {
          Request_Line_ID: 'SYNTHETIC-LINE',
          Request_ID: 'SYNTHETIC-REQUEST',
          Event_ID: 'SYNTHETIC-EVENT',
          Event_Item_ID: 'SYNTHETIC-EVENT-ITEM',
        },
      ],
      [
        context.HAU_SHEETS.DELIVERABLES,
        {
          Deliverable_ID: 'SYNTHETIC-DELIVERABLE',
          Assigned_Committee: 'Materials Committee',
          Request_Line_ID: 'SYNTHETIC-LINE',
          Event_Item_ID: 'SYNTHETIC-EVENT-ITEM',
        },
      ],
      [
        context.HAU_SHEETS.CANVASS,
        { Canvass_ID: 'SYNTHETIC-CANVASS', Linked_Deliverable_ID: 'SYNTHETIC-DELIVERABLE' },
      ],
      [
        context.HAU_SHEETS.COMPOSITE_REQUESTS,
        {
          Request_ID: 'SYNTHETIC-COMPOSITE',
          Component_ID: 'SYNTHETIC-FOOD-COMPONENT',
          Owner_Committee_ID: 'COM_FOOD',
        },
      ],
      [
        context.HAU_SHEETS.COMPOSITE_REQUESTS,
        {
          Request_ID: 'SYNTHETIC-COMPOSITE',
          Component_ID: 'SYNTHETIC-MATERIALS-COMPONENT',
          Owner_Committee_ID: 'COM_MATERIALS',
        },
      ],
    ];
    context.findOne_ = (sheet, key, value) =>
      records.find(([name, row]) => name === sheet && String(row[key]) === String(value))?.[1] || null;
    ...
    expect(
      context.authorizationResourceFromCommand_('confirmRelease', {
        lines: [{ requestLineId: 'SYNTHETIC-LINE' }],
      }).committeeIds,
    ).toEqual(['COM_FOOD']);
    expect(
    ...
    expect(
      context.authorizationResourceFromCommand_('transferEventItemToInventory', {
        eventItemId: 'SYNTHETIC-EVENT-ITEM',
      }).committeeIds,
    ).toEqual(['COM_FOOD', 'COM_MATERIALS']);
    expect(
      context.authorizationResourceFromCommand_('updateFoodComponent', {
        requestId: 'SYNTHETIC-COMPOSITE',
        componentId: 'SYNTHETIC-FOOD-COMPONENT',
      }).committeeIds,
    ).toEqual(['COM_FOOD']);
    expect(
      context.authorizationResourceFromCommand_('getCompositeRequest', { requestId: 'SYNTHETIC-COMPOSITE' })
        .committeeIds,
    ).toEqual([]);
  });
  ...
  it('blocks activation for every unresolved active mapping issue', () => {
      ...
      users: [
        {
          ...
          Role: 'ADMIN',
          Active: true,
          Authorization_Overrides_JSON: JSON.stringify({ grant: ['capability.not.real'] }),
        },
        {
          _row: 3,
    ...
    expect(
      report.issueRows.every((row) => !JSON.stringify(row).match(/SYNTHETIC|email|committee\.not/)),
    ).toBe(true);
  });

  it('treats blank and invalid active values as unresolved instead of active access', () => {
    const context = gasContext();
    const decision = context.authorizationDecision_(
      { User_ID: 'SYNTHETIC-INVALID-ACTIVE', Role: 'ADMIN', Active: 'MAYBE' },
      context.HAU_CAPABILITIES_.VIEW_AUDIT,
      {},
    );
    expect(decision.reason).toBe('MAPPING_RECONCILIATION_REQUIRED');
    expect(
      context.authorizationContext_({ User_ID: 'SYNTHETIC-BLANK-ACTIVE', Role: 'ADMIN', Active: '' })
        .mappingStatus,
    ).toBe('NEEDS_REVIEW');
  });
  ...
  it('applies a clean mapping only inside the lock with idempotency and audit hooks', () => {
    ...
    context.recordIdempotency_ = (_key, result) => {
      idempotencyCount += 1;
      return result;
    };
    context.resolveCurrentUser_ = () => users[0];

    const result = context.applyAuthorizationMapping_(
      { clientRequestId: 'SYNTHETIC-MAPPING-1' },
      'COR-SYNTHETIC',
    );
  ...
  it('reports dry-run mapping issues without changing records or exposing values', () => {
    ...
    const context = gasContext({
      users,
      memberships: [member('SYNTHETIC-STAFF', 'COM_FOOD'), member('SYNTHETIC-HEAD', 'COM_FOOD', false)],
    });
    const report = context.authorizationMappingDryRun_();
