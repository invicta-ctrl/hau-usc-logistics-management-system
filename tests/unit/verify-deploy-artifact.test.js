...
function artifact(mode) {
  return `<!doctype html><html><head><meta name="hau-deploy-target" content="${mode}"></head><body></body></html>`;
}
...
describe('deploy artifact verifier', () => {
  ...
  it('rejects absent or duplicate canonical markers', async () => {
    const absent = await verify('staging', '<!doctype html><html><head></head><body></body></html>');
    const duplicate = await verify(
      'staging',
      `<!doctype html><html><head>${artifact('staging').match(/<meta[^>]+>/u)[0]}${artifact('staging').match(/<meta[^>]+>/u)[0]}</head><body></body></html>`,
    );
  ...
  it('rejects a preview marker even when it has the canonical shape', async () => {
    const result = await verify('staging', artifact('preview'));

    expect(result.status).toBe(1);
    expect(result.output).toContain('invalid deploy target marker');
  });
});
