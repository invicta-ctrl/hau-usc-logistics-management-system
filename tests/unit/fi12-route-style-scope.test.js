...
describe('FI-12 route-style containment', () => {
  it.each(routeMounts)('contains local selectors below %s without losing its responsive root rule', (root) => {
    ...
    expect(scoped).toContain('@media(max-width:768px){:scope{padding:14px}:scope table{display:none}:scope th{display:none}:scope td{display:block}header{display:block}}');
