...
Status: v0.8.0 Slice 1 baseline
...
| `postCycleCountAdjustment`                         | One D1 batch, but see V080-S1-INV-03                   | `inventory.adjust`                                                       | Fingerprinted                           | `CYCLE_COUNT_ADJUSTMENT` plus audit                                                        | Source contract only; missing concurrency regression            |
| Event-item transfer                                | **Not implemented by Worker/D1**                       | Client contract claims it; server has no capability/handler              | None at D1 boundary                     | Schema can hold the intended pair, but current production call cannot post it              | V080-S1-INV-01                                                  |
| Standalone active-reservation cancellation/release | **Not implemented**                                    | No command/capability route                                              | None                                    | Schema already supports cancellation status                                                | V080-S1-INV-02                                                  |
...
| `runtime.js` `itemOnHand` / `availableToPromise`       | **PRESENTATION_ONLY** in production               | Production prefers server-supplied values; allocation display clamps negative ATP to zero. Mock fallback reduces local movement directions.              |
| `runtime-extensions.js` `inventoryBalance`             | **PRESENTATION_ONLY**                             | Production recomputes ATP as supplied on-hand minus supplied reserved. Its no-server fallback has V080-S1-INV-04.                                        |
| `src/domain/inventory.js` and `src/features/inventory` | **LEGACY_OR_DEAD** for the built production entry | Used by the alternate `src/main.js` app path and mock tests, not the authoritative `src/index.html` visual runtime. Its reducer also has V080-S1-INV-04. |
...
| Invariant                                    | Current boundary                                                                     | Existing/strengthened proof                                                                                               | Result                        | Gap / Slice 2 action                                                                  |
| -------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------- |
| INV-01 one physical truth                    | Schema-24 `inventory_balances`; ledger/update/delete/opening guards                  | `inventory-production-import`, `inventory`, Worker authoritative-balance test                                             | **PASS**                      | Preserve the D1 view and never add a mutable balance column.                          |
| INV-02 reservation does not change on-hand   | `reservations` + `reservation_consumptions`; reserve D1 batch has no ledger insert   | **Strengthened** Worker lifecycle asserts on-hand/ledger unchanged and reserved/ATP delta only                            | **PASS WITH LIFECYCLE GAP**   | V080-S1-INV-02: add governed active-reservation cancellation/release.                 |
| INV-03 request submission does not deduct    | `submitRequest` batch contains request/line/history/audit/idempotency only           | `public-request-v072-contract`; **strengthened** internal Worker lifecycle                                                | **PASS**                      | None.                                                                                 |
| INV-04 consistent availability               | Schema view exact formula; lending view derives from it                              | `inventory`, production-import, Worker lifecycle, RV-01 top-up/concurrency                                                | **PASS**                      | V080-S1-INV-04 applies only when server values are absent.                            |
| INV-05 explicit movements/history            | Receive, release, correction, lending, return, opening, adjustment D1 batches        | Worker lifecycle, cumulative receiving, traceable assets, integration audit probes                                        | **PASS WITH TRANSFER GAP**    | V080-S1-INV-01: implement the claimed transfer command as an atomic ledger pair.      |
| INV-06 retry safety                          | `idempotency_keys` actor+fingerprint replay; per-entity unique keys                  | Worker reserve/release/correction/lending/receiving tests and integration audit probes                                    | **PASS**                      | Keep effect and replay record in the same batch.                                      |
| INV-07 authorization/privacy                 | Worker auth/CSRF, method capability, entity scope; item DTO redaction                | Authorization, operational-scope, public Request/Lending, request-only Worker tests                                       | **PASS**                      | Preserve server-only capability enforcement and requester redaction.                  |
| INV-08 append-only history                   | Ledger/status/audit/consumption/classification/correction/asset/restore triggers     | `cloudflare-phase3`, production-import, Worker correction/asset tests                                                     | **PASS**                      | Do not update/delete original history; corrections remain linked additions.           |
| INV-09 no negative/impossible accepted state | Capacity/nonnegative/receiving/correction/asset guards plus state/unique constraints | RV-01 over-reservation and concurrent reservation tests; Worker duplicate release/handoff/return and over-receiving tests | **PASS WITH CONCURRENCY GAP** | V080-S1-INV-03: guard cycle-count adjustment against a stale concurrent on-hand read. |
| INV-10 Google not authority                  | Worker -> D1/R2 architecture; Apps Script/Google isolated as sidecar/recovery        | Architecture contract, Cloudflare Worker/D1 tests, request-only operation with no Google dependency                       | **PASS**                      | No Google fallback may be added.                                                      |
