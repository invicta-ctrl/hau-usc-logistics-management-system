...
Status: active adoption record for the Isolated Staging Playground amendment.
...
## Recovery-pointer source plan

Before any remote branch write, prove the peeled immutable source and release/recovery status for:

- `backup/last-known-good` <- v0.7.2 accepted production;
- `regression/r1` <- v0.7.1 accepted production;
- `regression/r2` <- v0.7.0 accepted production;
- `regression/r3` <- preserved canonical pre-v0.7.0 checkpoint, accepted only after historical recovery proof.
