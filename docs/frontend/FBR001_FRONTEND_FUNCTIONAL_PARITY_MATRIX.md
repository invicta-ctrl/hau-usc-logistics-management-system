...
| `inventory` and catalogue | Authorized inventory/read/classification; audited changes only | Inventory bootstrap; ledger contract; availability/on-hand, empty/error/stale | Inventory stock/restock/catalogue tabs, edit/add controls | PRESENT — F2 applies the Workbench route boundary, deliberate mobile table region, focus treatment, and authoritative stale/error/denied states. Direct reserve/adjust/restock-transition controls remain intentionally deferred: no accepted React route command exists. |
| `restocking`/receiving | `fulfillment.receive`; evidence where granted | `/api/bootstrap/restocking`, `/api/receiveRestock`, evidence upload; cumulative/idempotent/conflict | Receiving is visually inside Claude Inventory | PRESENT — F2 preserves the dedicated canonical route, current receiving station, evidence, cumulative/idempotent/conflict handling, and Workbench body boundary. |
| `request-center` Internal Request Hub | `request.review` and line-level route capabilities | Internal request bootstrap, `/api/reviewRequest`; per-line routing, correction/reject reasons, split/stale/denied | Request Hub queue, drawer, per-line decisions | PRESENT — F2 retains queue/detail/per-line review and current conflict/denial refresh, replacing the selected-row side stripe with a semantic top decision rule. |
| `release` Release Desk | `fulfillment.release`, evidence upload | `/api/bootstrap/release`, `/api/confirmRelease`, `/api/uploadEvidence`; ready-to-claim/custody/history | Claude puts release inside request/loan records | PRESENT — F2 preserves the dedicated canonical deep link, custody/evidence station, authoritative recheck, history, and Workbench route boundary. |
| `lending` Internal Lending Hub | Lending approval/handoff/return/evidence capability projection | Current lending bootstrap and approve/handoff/return/evidence commands; custody, due/overdue, duplicate/conflict | Lending Hub and record sheet | PRESENT — F2 retains lifecycle/evidence/custody states and capability-gated actions; filter/lifecycle tabs now scroll deliberately on narrow widths. |
| `procurement` | Current procurement/canvass capabilities | `/api/bootstrap/procurement`; current command set; quote/award/error/denied | Procurement requisitions, canvassing and suppliers | PRESENT — F2 preserves current server-backed canvass/deliverable presentation in the shared route body. Command mutations remain intentionally deferred: no current accepted React action exists. |
| `events` | `event.manage` where granted | Current events bootstrap/event management; loading/error/denied | Events cards, readiness context and optional event themes | PRESENT — F2 retains current readiness/activity/empty/error/denied presentation in the shared route body. Event mutations and live themes remain intentionally deferred without accepted route commands/data. |
