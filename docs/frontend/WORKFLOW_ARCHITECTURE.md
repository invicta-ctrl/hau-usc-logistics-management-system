...
| FE-R3-010 | P2 | The Impeccable sidecar `.impeccable/design.json` is stale and misreports. It was last written 2026-08-08 by the pre-cutover v4.1 redesign (`a413824`); `DESIGN.md` was rewritten 2026-08-22 by the Figma-native cutover (`f7e5bf8`). Its 15-colour palette contains **none** of the current identity anchors, so the `design-system-color` rule flags oxblood `#40070a` and gold `#e8b93c` as drift when `DESIGN.md` and `PRODUCT.md` both name them as the institutional identity. Most current `design-system-color` findings are therefore false positives against a superseded system. Refreshing the sidecar (`/impeccable document`) is a design-authority write and was deliberately not performed unbidden. | FI-13 |
| FE-R3-011 | P3 | Separately from the stale sidecar: the frontend does use literal hex/rgba throughout rather than named tokens, faithful to the Make source. Genuine tokenization is a large refactor and should follow the sidecar refresh, not precede it. | FI-12 / FI-13 |
