...
| FE-R3-003 | P2 | **Closed by MFR-002 U04.** Public Lending no longer renders a second branded masthead or duplicate Home/Staff Sign In exits. Its local navigation owns only Lending, Tracking, and Policy; the persistent shell owns global exits. | CLOSED |
...
| FE-R3-008 | P3 | **Closed in the current landing implementation.** Loading, empty, request-error, and media-error copy each render in one state owner. | CLOSED |
...
| FE-R3-011 | P3 | **Closed by MFR-002 U02 + U04.** U02 generated the semantic type/radius foundation; U04 migrated Public Lending's remaining route-local radius declarations to the shared surface, overlay, and pill roles. | CLOSED |
...
| FE-R3-013 | P2 | **Closed by MFR-002 U04.** `AccountAccessPanel` and `AccountRecoveryPanel` were migrated together to generated semantic theme roles across every Light/Dark appearance. Literal values remain fallbacks only. | CLOSED |
| FE-R3-014 | P2 | **Closed by MFR-002 U04.** Public Lending, External Request, and account-lifecycle selects now use separate explicit `label[for]`/control associations so option text cannot contaminate accessible names. | CLOSED |
