...
- The playground Worker is accepted only after same-origin `/api/version` and `/api/health` prove `STAGING`, the server-owned playground flag, D1 readiness, and both R2 dependencies. Browser input cannot turn the Index or playground controls on. In that verified environment only, `POST /api/playground/session` selects an active, onboarded, unlocked System Owner from the isolated D1 on the server and returns an audited session; the browser cannot submit an account, role, capability, D1, or R2 selector. Production returns 404 before auth/binding setup.
...
| `public.landing`            | `BACKEND_READ_ONLY`                    | The first active PUBLIC advertisement projects the current major event's title, summary, CTA, destination, alternative text, and authorized media URL over the vendored official USC fallback; portal navigation remains client-side     |
...
| `public.verify`             | `V5_NATIVE_FUNCTIONAL_PARITY_ADDITION` | Account-application email start/confirm with exactly eight decimal digits and an opaque verification receipt; expiry, resend limits, one-time use, attempt controls, digest-only storage, non-enumeration, and audit remain server-owned |
...
- `src/v5/integration/backend.js` owns the same-origin auth/API/legacy-command adapters and stores authenticated CSRF state after login, starter activation, or the guarded playground-only owner session.
...
- Browser: all 33 registered V5 routes plus playground Index/search, guarded owner auto-session, real-login/resume path, dynamic event projection, responsive widths `320/375/390/414/768/1024/1280×800/1440/1920`, first-fold fit, light/dark, and Production absence of playground UI/session activation.
