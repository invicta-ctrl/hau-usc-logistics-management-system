...
The compact scoped-revision endpoint re-authorizes the requested active module and returns only contract/enabled state, scope token, global revision, update time, environment, and read metrics—never operational rows. Unknown, unauthorized, or request-only scopes fail closed. The request-only bootstrap omits revision fields and the requester UI does not start the internal polling controller. Request-only users do not receive catalog-management permissions or internal circulation policies/balances.
