...
| Spreadsheet formula injection from user text                          | Sheet owner/session and exported data      | No complete repository-wide formula-like input control is evidenced                                        | **Launch gap:** reject or safely encode values beginning with formula control characters before any Sheet write/export; add tests                |
...
| Malicious or mislabeled upload                                        | Operator device, Drive, downstream viewers | MIME allowlist, extension match, size limit, digest                                                        | **Launch hardening:** signature validation, malware/PDF active-content scanning, image bounds, viewer guidance                                   |
...
- Never trust client file MIME, size, filename, entity relationship, or digest. The server computes size/digest and must add signature inspection before broad production.
