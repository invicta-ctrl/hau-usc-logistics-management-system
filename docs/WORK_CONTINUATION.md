...
- **Branch/HEAD/upstream:** `integration/v0.5-baseline`; accepted Slice 9 starting HEAD and upstream `a0b4e7f46620056b5fc73036bea69bfcf8da0f83`; local/upstream `0 0` before the current uncommitted implementation.
- **Current phase/stage:** Phase 4 / Slice 9 Venue and Equipment reference/request vertical; `SLICE_9_LOCAL_GATES_AND_INDEPENDENT_REVIEW_PASS_READY_COMMIT`.
- **Accepted scope:** additive versioned references/routes, bounded safe lookup, venue/equipment selection, quantity/unit/summary, constrained Other triage, truthful effective requestability, office/authority/instructions/lead time, server-selected existing-committee routing, immutable child snapshots, confirmation/blocker/return/evidence state, scoped queue/update, authorization/revision/idempotency, active UI, tests/generated parity/docs, focused push, and CI.
- **Completed work:** Slice 8 is fully accepted. Slice 9 now implements additive empty live reference/route tables, effective exact validation, bounded safe search, constrained Other, route-owned children, immutable snapshots/provenance, confirmation/blocker/return/evidence controls, scoped queue/update APIs, adapters, active request/management UI, synthetic preview fixtures, focused tests, and generated parity.
- **Files changed by purpose:** contract/checkpoint docs; Venue and Equipment domain/server/schema/router/authorization; composite/mock/adapters/config; source runtime UI; focused unit/browser tests; generator/check updates; regenerated standalone and Apps Script partials. `PROJECT_STATUS.md` and `CHANGELOG.md` record the handoff.
- **Tests verified at current SHA:** governance and lint pass; 32 Vitest files / 262 tests pass; build transforms 32 modules; 32 Apps Script sources / 54 functions and deterministic parity pass; standalone verification passes; full Playwright passes 61 / 101 intentional skips / 0 failed; sensitive scan and `git diff --check` pass. Final independent review is PASS after all targeted repairs.
- **Generated artifacts:** both standalone files are 365,301 bytes / SHA-256 `d26b8c224a7509e0a114f2f2e2f31e789391750a572b53d648452acbe02c91d6`; Apps Script `Index.html` is 1,022 bytes / SHA-256 `bf52d20bbbc8d2a35b39351500edf7c65db93ab2dbddb18cd77c1587389d6035`; `AppScript.html` is 301,632 bytes / SHA-256 `7fbbeadc3c5a71a5f9a56a501db266f099569706da540483575228995d08838d`.
- **External actions:** no Slice 9 external action. No merge, `main` update, deployment, Apps Script push, Sheet/Drive write, migration/import, Cloudflare, database, staging, or production action.
...
- **Rollback:** set `HAU_VENUE_EQUIPMENT_REQUESTS_ENABLED=false` to stop new specialized selection while retaining stored versioned children/history and safe fallback; use a focused Slice 9 revert without resetting or discarding unrelated work.
...
- **Next three actions:** run the final complete check/diff/privacy audit; commit/push Slice 9; verify PR #7 CI before Slice 10.
