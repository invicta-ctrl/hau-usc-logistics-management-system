...
- **Branch/HEAD/upstream:** `integration/v0.5-baseline`; source checkpoint `4211711c78ce382a54b57a96aca01d17252ffd6c` is pushed at parity and PR #7 checks are green.
...
- **Files changed by purpose:** the progressive bootstrap read DTO now applies the established fail-closed legacy handling/lending mapping; its regression fixture covers the unresolved placeholder; safe repository status documents record the live recovery checkpoint. Private IDs, contacts, configs, and evidence remain outside Git.
- **Tests verified at current SHA:** source checkpoint `4211711` passes `npm run check` with 39 Vitest files / 323 tests, build, 33 Apps Script sources / 55 functions, deterministic generated parity, standalone verification, guided demo, seven module shareables, and full Playwright. PR #7 `validate`, `verify`, and `browser-smoke` pass.
...
- **Remote evidence:** 39/39 exact pull-back parity, owner-only manifest, isolated repair Version 25, accepted current demo recovered to Version 13, distinct verified backup Version 12, five reconciled access rows, zero duplicate IDs, four mapped active rows, and one inactive synthetic row. Version 18 is retained as incident evidence only.
...
- **Next three actions:** add the two distinct institutional test identities; rerun AUTH-01/FUNC-01 and the remaining identity-independent Gate E rows on isolated Version 25 while keeping near-live manual-only; promote a repaired candidate only after every must-pass row is signed.
