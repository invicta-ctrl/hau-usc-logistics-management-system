...
- **Current phase/stage:** Phases 0–16 accepted on staging; Phase 17 Inventory Production Data Readiness has a focused safe-classification local pass and awaits full/staging acceptance; production NO-GO.
- **Accepted scope:** Phase 17 of `.codex/specs/v0.7.0-production-master.md` under the follow-up amendment, hybrid evidence-storage amendment, owner-approved `.codex/specs/v0.7.0-phase-17-safe-classification-amendment.md`, and shared efficiency contract.
- **Completed work:** Phase 16 remains accepted. Phase 17 captured and validated a private immutable 397-row source snapshot, repaired ledger-only opening imports and duplicate quarantine, proved isolated import/replay reconciliation, and implemented schema 25 plus the protected audited in-website classification workflow with fail-closed lending.
- **Tests verified:** Current `npm run check` passes 71 files / 463 tests and every repository gate. Isolated schema-25 Worker/D1 passes 31 / 31, full browser coverage passes 126 with 306 intentional skips, focused Inventory queue/API/browser evidence passes, and the prior 397-row import/replay reconciles exactly.
...
- **Blocker:** no owner-data blocker remains for safe import; all pending rows must remain visibly tracked and fail closed. Staging is unchanged until the current full gates, diff review, candidate checkpoint, and private backup pass.
- **Next action:** review/commit/push the candidate, recapture the candidate-bound private source snapshot, then back up and apply migrations 0024–0025 plus import/replay on staging with full reconciliation.
...
- **Completed work:** canonical governed lending catalog, borrower-safe/staff separation, one availability model, reusable assets, controlled asset lifecycle, and bounded catalog-image delivery.
- **Files changed by purpose:** Phase 5 migration/catalog service, D1 operations, public portal, Worker route, tests/generated artifacts, `.codex/V0_7_PHASE_5_LENDING_CATALOG_HANDOFF.md`, and canonical status records.
- **Tests verified at current SHA:** `npm run check` (58 files / 401 tests), local Worker/D1 18 / 18, full Playwright 94 / 224 intentional skips, remote CI 6 / 6, deployed staging 4 / 4, and cache-busted health/readiness/version.
- **Generated artifacts:** standalone, guided demo, seven module shareables, and Apps Script bundle were regenerated by `npm run check` and passed parity/verification.
- **External actions:** staging migration 0014 and exact-SHA Worker deployment completed; one audited synthetic lending fixture was activated only for smoke and re-archived; production and Google sources were untouched.
- **Rollback:** private pre-0014 export SHA-256 `27a724b944b7846606af6faefe762245e3840ddab80411197d0a766b7d6b68cc`; prior immutable Worker retained; formal rehearsal remains Phase 24.
- **Blocker:** none for Phase 6 implementation; real public-lending eligibility and approved event values remain absent and must not be invented.
- **Next three actions:** audit the existing internal lending UI/backend against Phase 6; implement only missing review/partial/substitution/issue/return exception contracts; verify locally and on staging.
...
- **Frozen candidate:** `62abc6d1e1d6b3079e8508381b7c336c636080e5`; exact Phase 3 implementation range `38a86069039ef18081aaa0e1c1fe2c25acde6613..62abc6d1e1d6b3079e8508381b7c336c636080e5`. It was pushed at branch/upstream/PR parity; draft PR #9 was open and mergeable and all six checks passed on 2026-07-22.
...
- **Smallest safe next action:** create and approve an outside-Git Phase 3 authorization package bound to the frozen candidate SHA/artifact/Worker/mapping/migration hashes. Validate Gate B before the first `wrangler whoami` or Google read. Never print private values.
...
- **Completed scope:** the shared shell; all five role experiences; Request Center; Lending Hub; Release Desk; Inventory/Restocking; Canvass; Procurement/Deliverables; inherited login/onboarding; responsive behavior; 13 deterministic review artifacts; and the minimum system, role, request, lending, and demo guides.
- **Final verification:** `npm run check` passes with 49 Vitest files / 356 tests, deterministic build/parity, Apps Script validation, and standalone verification. Opt-in preview generation passes 3 / 3 with 3 intentional project skips. Normal complete Playwright passes 90 / 204 intentional project/viewport skips / 0 failures without changing tracked PNG hashes. `dist/index.html` is 455,779 bytes / SHA-256 `369ef83f8cdfe520049ae26fc853e70072ed54f9196a2899adff09dbd93ea8ed`.
- **Remote evidence:** draft PR #9 remains open and mergeable at `de194f5`; `validate` (11s), `verify` (18s), `build` (24s), `report-build-status` (3s), automatic `deploy` (8s), and `browser-smoke` (3m08s) pass. The Pages deploy is automatic repository CI behavior, not a manual production promotion.
- **Known gates:** the Administrator contrast defect found in final visual review is fixed and regression-tested. Phase 3 still owns Worker/D1 persistence, distributed controls, migration/reconciliation, integration hardening, and final program acceptance. Venue/Equipment activation still needs an approved external reference/routing list. `npm run extract:visual` has known drift and must not be accepted without reviewing its diff.
- **External boundary:** no manual deployment, Apps Script push, Sheet/Drive/institutional write, Cloudflare/D1 action, migration, production promotion, `main` update, or PR merge occurred.
- **Next action:** manually switch to GPT-5.6 Sol High in a fresh task; perform the full Git/PR/CI handshake; read `.codex/PHASE_2_TERRA_HANDOFF.md`; then read the Phase 3 specification and report whether its start condition is satisfied before implementation.

- **Gate 6 shared operational workflows checkpoint:** code milestone `478c2feef3040469c820f356ec8a329a32fbc606` is pushed on `chore/v0.6-codex-continuity-bootstrap`; draft PR #9 head matches and all remote checks pass.
- **Release acceptance:** the handoff form retains selected lines and quantities, recipient name/role/department, notes, releasing staff/time, required recipient confirmation, and optional photo capture/upload. The Apps Script service validates metadata, scope, duplicate lines, status, remaining quantity, aggregate balances, and reservations before mutation; lock, authorization, idempotency, history, audit, and append-only issue ledger behavior remain intact.
- **Inventory/Restocking acceptance:** inventory truth remains derived from immutable ledger movements and active reservations, with on-hand/reserved/ATP separation, aliases/search, category/storage/handling/unit, reorder thresholds, provenance, history, and archive/restore. The restock queue retains all accepted statuses, preferred-quote/revision guards, immutable receipts, sibling isolation, and cumulative line-level receiving.
- **Canvass/Procurement acceptance:** search, filters, sorting, pagination, comparison, preferred quotes, price history, stale/missing-unit/unit-mismatch indicators, and safe evidence/source links are available. Deliverable receiving is allowed only from `PROCURED`/`PARTIALLY_RECEIVED`; the form posts quantity received now and displays cumulative received/remaining/total values through repeated receipts.
- **Verification:** `npm run check` passes with 49 Vitest files / 356 tests, deterministic generated parity, Apps Script validation, and standalone verification. Focused Gate 6 Playwright passes 6 / 6 at 390px and 1366px. Full `npm run test:e2e` passes 90 / 186 intentional skips / 0 failures. `dist/index.html` is 455,785 bytes / SHA-256 `1fc85c15b361136824ab9c8156b67b30607dcb2157a3bbfc21a4155fd02240e8`.
- **Remote evidence:** PR #9 remains open, draft, and mergeable at `478c2fe`; `validate` (15s), `verify` (20s), `build` (24s), `report-build-status` (4s), automatic `deploy` (10s), and `browser-smoke` (3m7s) pass.
- **External boundary:** no manual deployment, Apps Script/Sheet/Drive/institutional write, Cloudflare/D1 action, migration, production promotion, `main` update, or PR merge occurred.
- **Next action:** create and inspect the required deterministic experience previews, then update the minimum authoritative user/role/demo guides and produce the final Phase 2 handoff. Do not begin Phase 3.

- **Gate 5 Request Center and Lending Hub checkpoint:** code milestone `09315fbb68119aa55e16bba26b4926b60b3dfba9` completes the accepted Phase 2 Gate 5 scope without changing Phase 1 authentication, authorization, transaction, ledger, or rollback contracts.
- **Request Center acceptance:** Event Logistics and Catalog Restock remain available; Event Step 4 exposes Food, Materials, and Venue & Equipment together; untouched sections create no child; additional-request IDs are filtered to the selected event; the combined date range, predictive item search, and Issue/Canvass/Split presentation are preserved; submission creates a review record without ledger or reservation movement.
- **Lending acceptance:** new Student IDs are digits-only with a maximum of eight at client and Apps Script service boundaries. Moving `FOR_REVIEW` to `READY_TO_CLAIM` requires a `Can_Review` user to attest `APPROVED_ACTIVE_USC_SOURCE` for USC officers/staff or `APPROVED_ANGELITE_IDENTITY_RULE` for Angelite/students. The approved source, reviewer, and verification time are recorded in append-only history/audit metadata; an email domain is never treated as verification.
- **Preserved behavior:** form/ticket split, loan and consumable lifecycles, predictive catalog eligibility, stock revalidation, reservations, idempotency, duplicate handoff/return prevention, controlled ledger movements, and the accepted security/privacy boundary remain unchanged.
- **Gate 5 verification:** `npm run check` passes with governance, lint, 46 Vitest files / 348 tests, deterministic build/parity, Apps Script validation, and standalone verification. Focused Request/Lending/composite Playwright passes 29 / 31 intentional skips across configured viewports. `dist/index.html` is 448,084 bytes / SHA-256 `a4698f75f7c2b2bb4129be0f7c655945ed40d0d9c8c9f8540a8401de876e839c`.
- **External boundary:** no Apps Script deployment, Sheet/Drive write, institutional-data write, Cloudflare/D1 action, migration, production promotion, `main` update, or PR merge occurred.
- **Next action:** push and verify the Gate 5 checkpoint and PR #9 CI, then audit and complete only Gate 6 Release Desk, Inventory, Restocking, Canvass, and Procurement acceptance.

- **Materials & Documentation checkpoint:** code milestone `67c542263431c813cb5ae6488cbdacf38e74fbae` implements the S0002 Materials experience inside the shared role layer. `MATERIALS.html` was read at SHA-256 `0c7b82d130470772c1045dc53a3c1810155b15274cf173c3453fbbb6d1bd09e5` and its durable decisions were appended to `.codex/DESIGN_REFERENCE_DIGEST.md`.
- **Materials scope and boundaries:** canvassing, budget, procurement, and release-readiness signals lead to existing shared workspaces while exact specification, request/event identity, quote freshness/evidence, cumulative receiving, deliverable quantities, and provenance stay connected. The blue accent and Materials ownership grant no authority; quote preference, purchase, receipt, stock transfer, and controlled release remain capability-bound and server-validated.
- **Materials verification:** focused role unit tests pass 4 / 4; the combined responsive role proof passes 8 / 16 intentional skips, including Materials at 390px and 1366px with no horizontal overflow; `npm run check` passes with governance, lint, 46 Vitest files / 345 tests, deterministic build/parity, Apps Script validation, and standalone verification. `dist/index.html` is 444,691 bytes / SHA-256 `ba6d195929efdc78e602c9441bda2ea3b17fa71f699bfe7192e2258bb9863f71`.
- **Next action:** push the Materials & Documentation checkpoint and verify PR #9 CI, then audit and complete only Gate 5 Request Center and Office Lending Hub acceptance.
