...
- **Branch/HEAD/upstream:** Resolve with `git branch --show-current`, `git rev-parse HEAD`, and `git rev-parse --abbrev-ref --symbolic-full-name @{upstream}`; canonical closeout must merge through protected `main`.
...
- **External actions:** Read-only GitHub, production runtime, and staging runtime/route/status checks only. No provider send, staging mutation/reset/seed/deploy, production mutation/deploy, tag, release, or database write.
