...
- Local HEAD: `b8beda10e6271fbbe91d3b0f0b04e4c0828e4c21`; upstream: `origin/feat/live-sync-lending-search-catalog-controls`; local/upstream count: `4 0`; worktree is clean.
- The two local Apps Script/bootstrap repair commits and the documentation rollback checkpoint are complete and remain unpushed because final v2 staging acceptance has not passed. The remote branch remains at `3f038590bdebe5b117018356838b82d94683ad37`.
...
- Current stage: `PHASE_E_V2_ROLLED_BACK_PROPERTY_RESTORED_LOCAL_DIAGNOSIS`. The staging owner set `HAU_BOOTSTRAP_CONTRACT_VERSION=2` while authorization contract v1/absent remained in force. Direct v2 endpoint checks passed, but the live v2 workspace stayed in slow startup and reached the retryable read-only-service timeout instead of ready.
...
- Do not use the production-target local configuration, add a temporary setter, or create a temporary deployment. Diagnose and repair the v2 timeout locally before another staging deployment. Do not claim the acceptance marker or begin Slice 4.
