...
| MK-03 | MEDIUM | Make route files paint with literal hexes instead of reading the theme tokens, so `theme.css` adoption cannot fully retheme them | OPEN — measured on live v36: `PublicFlows.tsx` 6 superseded literals of 44, `LendingHubRoute.tsx` 3 of 18, `ReleaseDeskRoute.tsx` 3 of 17, and **`RequestCenterRoute.tsx` 21 of 72 while reading zero CSS variables** — its colours come from a local `ap(dark)` helper and it carries 13 occurrences of `#e8b93c`. **Correction 2026-08-20:** the earlier note implied only the 21 were blocked and the other 12 were actionable. They are not. Any Make edit joins the same pending save set, so all 33 are held by the same `WHOLE_PROJECT_SAVE_ONLY` boundary, not by two different reasons. Status for the whole set is `READY_AFTER_OWNERSHIP_RESOLUTION`. Adoption packet §4 |
...
| MK-04 | LOW | The per-line route select in the unsaved `RequestCenterRoute.tsx` buffer does not match production's derived route set | OPEN, **not ours to fix** — **re-derived from Product truth 2026-08-20.** Production does not carry a fixed list: `permittedRoutes(request, line)` in `src/visual/runtime.js` makes only `REJECT` and `MISSING_INFORMATION` unconditional, gates `ISSUE_FROM_STOCK` and `RESTOCK` on `line.itemId`, and gates `RESTOCK` further on `request.type === 'CATALOG_RESTOCK'` or a `OFFICE_INVENTORY`/`PANTRY` `catalogType`. The Make fixture has **none of those three fields**, so the option set cannot be derived there at all and the current four-option list is underdetermined rather than missing one. The fix is to add the discriminating fields and derive the options, **not** to add Catalog restock unconditionally. Exact replacement code in adoption packet §4.1. Lands in the ownership-blocked file |
