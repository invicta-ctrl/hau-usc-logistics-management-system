...
| MK-06 | LOW | Make landing sections below the hero may not respond to dark mode | **OPEN, observed not diagnosed.** With the theme in dark mode, the "What the council is doing now" section rendered on a near-white surface. The likely cause is structural rather than a defect in this adoption: before it, Make's brand palette had no dark values at all, so landing surfaces were mode-invariant by accident and nothing below the hero was ever authored against a dark palette. Not investigated further, and deliberately not patched — whether the Make landing should respond to dark mode at all is a design decision, not a token fix. One file to check: `src/app/landing/CurrentSection.tsx` with `src/styles/index.css` |
