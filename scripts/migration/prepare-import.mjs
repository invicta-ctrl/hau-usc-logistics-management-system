...
function convert(raw, rule, importedAt) {
  ...
  if ((value === undefined || value === null || value === '') && Object.hasOwn(rule, 'default')) value = rule.default;
  if (rule.required && (value === undefined || value === null || String(value).trim() === '')) throw new Error('REQUIRED_VALUE_MISSING');
  if (rule.type.startsWith('nullable-') && (value === undefined || value === null || String(value).trim() === '')) return null;
...
function insertStatement(mapping, values) {
  ...
  if (mapping.mode === 'append-only' || mapping.mode === 'insert-only') return `${base} ON CONFLICT DO NOTHING;`;
...
async function main() {
  ...
  for (const [name, value] of Object.entries({ input, output, rejectionsOutput, reconciliationOutput })) {
    if (!value || !path.isAbsolute(value)) throw new Error(`${name} must be an absolute path outside the repository.`);
  ...
  const requestLineItems = new Map(
    (source.tabs['04_REQUEST_LINES'] ?? []).map((row) => [String(row.Request_Line_ID ?? '').trim(), String(row.Item_ID ?? '').trim()]),
  );
  ...
  const orderedTabs = [
    '01_ITEM_MASTER', '13_EVENTS', '03_REQUESTS', '04_REQUEST_LINES', '06_LENDING',
    '05_RESERVATIONS', '11_SUPPLIERS', '08_RESTOCK', '09_DELIVERABLES', '10_CANVASS',
    '12_EVIDENCE', '07_RELEASES', '02_LEDGER', '15_STATUS_HISTORY', '16_AUDIT_LOG', '14_USERS_ACCESS',
  ];
  for (const tab of orderedTabs) {
    ...
    for (let index = 0; index < sourceRows.length; index += 1) {
      ...
      const sourceFingerprint = sha256(JSON.stringify(Object.keys(row).sort().map((key) => [key, row[key] ?? null])));
      ...
      try {
        if (mapping.transform === 'release-lines') {
          ...
          if (!sourceRecordId || !Array.isArray(lineIds) || !lineIds.length || lineIds.length !== quantities.length || lineIds.length !== units.length) {
          ...
          lineIds.forEach((lineId, releaseIndex) => {
            ...
            statements.push(insertStatement(mapping, values));
          });
          imported.push({ tab, rowNumber: index + 2, sourceRecordId, sourceFingerprint, targetTable: mapping.targetTable, targetId: sourceRecordId });
          continue;
        }
        const values = Object.fromEntries(
          Object.entries(mapping.fields).map(([target, rule]) => [target, convert(row[rule.source], rule, importedAt)]),
        );
        if (tab === '03_REQUESTS') {
        ...
        if (tab === '05_RESERVATIONS' && !values.idempotency_key) values.idempotency_key = `IMPORT:${values.id}`;
        if (tab === '10_CANVASS' && !values.idempotency_key) values.idempotency_key = `IMPORT:${values.id}`;
        if (tab === '15_STATUS_HISTORY' && !values.idempotency_key) values.idempotency_key = `IMPORT:${values.id}`;
        if (tab === '16_AUDIT_LOG' && !values.correlation_id) values.correlation_id = `IMPORT_${values.id}`.replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 64);
        if (tab === '08_RESTOCK') values.received_quantity = row.Received_At ? values.requested_quantity : 0;
        statements.push(insertStatement(mapping, values));
        if (tab === '01_ITEM_MASTER') {
          const aliases = String(row.Aliases || '').split(/[|,;]/).map((value) => value.trim()).filter(Boolean);
          aliases.forEach((alias) => statements.push(
            `INSERT INTO item_aliases (item_id, normalized_alias, display_alias) VALUES (` +
              `${sql(values.id)}, ${sql(alias.toUpperCase().replace(/[^A-Z0-9]+/g, ' ').trim())}, ${sql(alias)}` +
        ...
        imported.push({ tab, rowNumber: index + 2, sourceRecordId, sourceFingerprint, targetTable: mapping.targetTable, targetId: String(values[mapping.primaryKey]) });
      } catch (error) {
        rejections.push({ tab, rowNumber: index + 2, sourceRecordId, sourceFingerprint, reasonCode: error.message });
      }
    }
  }
  for (const record of imported) {
    statements.push(
      `INSERT INTO imported_source_rows (` +
        ...
        `) VALUES (` +
        [batchId, record.tab, record.rowNumber, record.sourceRecordId, record.sourceFingerprint, record.targetTable, record.targetId, 'IMPORTED', importedAt].map(sql).join(', ') +
        `) ON CONFLICT DO NOTHING;`,
    );
  }
  for (const record of rejections) {
    statements.push(
      `INSERT INTO imported_source_rows (` +
        ...
        `) VALUES (` +
        [batchId, record.tab, record.rowNumber, record.sourceRecordId, record.sourceFingerprint, validation.mapping.tabs[record.tab].targetTable, '', 'QUARANTINED', record.reasonCode, importedAt].map(sql).join(', ') +
        `) ON CONFLICT DO NOTHING;`,
    );
  }
  statements.push(
  ...
  await writeFile(rejectionsOutput, `${JSON.stringify({ batchId, snapshotHash: validation.snapshotHash, rejections }, null, 2)}\n`, { flag: 'wx', mode: 0o600 });
