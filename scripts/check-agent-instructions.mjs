...
export function validateAgentInstructions(text) {
  const required = [
    ...
    ['TOKEN-OPT A8 authority', /TOKEN-OPT-001-A8 is the active account-wide routing authority/i],
    ['Sol orchestrator', /ORCHESTRATOR_MODEL:\s*GPT-5\.6 Sol/i],
    ['Sol writes forbidden', /ORCHESTRATOR_WRITES:\s*FORBIDDEN/i],
    ['Sol subagents prohibited', /SOL_SUBAGENTS:\s*PROHIBITED/i],
    ['no mandatory zero-worker start', /may choose no workers or multiple direct workers/i],
    ['Luna Max cap 16', /MAX_LUNA_MAX_SUBAGENTS:\s*16\b/i],
    ['Terra Max cap 2', /MAX_TERRA_MAX_SUBAGENTS:\s*2\b/i],
    ['Ox Alpha cap 16', /MAX_OX_ALPHA_SUBAGENTS:\s*16\b/i],
    ['total direct worker cap 16', /MAX_TOTAL_DIRECT_SUBAGENTS:\s*16\b/i],
    ['delegation depth one', /DELEGATION_DEPTH:\s*1\b/i],
    ['recursive child spawning forbidden', /RECURSIVE_CHILD_SPAWNING:\s*FORBIDDEN/i],
    ['automatic model fallback disabled', /AUTOMATIC_MODEL_FALLBACK:\s*DISABLED/i],
    ...
    ['account-wide writer cap', /MAX_ACTIVE_WRITERS_ACCOUNT_WIDE:\s*2\b/i],
    ['one writer per worktree', /MAX_WRITERS_PER_REPOSITORY_OR_WORKTREE:\s*1\b/i],
    [
      'one frontend Terra writer',
      /FRONTEND_WRITER:\s*exactly one GPT-5\.6 Terra \/ Max when implementation is required/i,
    ],
    ['frontend Luna read-only', /FRONTEND_LUNA:\s*READ_ONLY/i],
