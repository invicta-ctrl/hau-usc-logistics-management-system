...
const requiredFunctions = [
  ...
  'api_getAuthorizationContract',
  'runAuthorizationMappingDryRun',
  ...
  'api_createInventoryItem',
  'api_updateInventoryItem',
  'api_updateInventoryStorageContext',
  'api_archiveInventoryItem',
  'api_restoreInventoryItem',
  'api_htmlDiagnosticPing',
  'htmlTemplateDiagnostics',
  'api_runRosterSync',
  'api_getRosterSyncHealth',
  'api_setRosterEmergencyDeny',
  'setupRosterSyncTrigger',
  'runScheduledRosterSync',
  'committeeDashboard_',
  'api_submitCompositeRequest',
  ...
  'api_amendCompositeRequest',
  'api_addCompositeSection',
  'api_assignCompositeComponent',
  'api_escalateCompositeComponent',
];
...
const missingFunctions = requiredFunctions.filter(
  (name) => !new RegExp(`function\\s+${name}\\s*\\(`).test(combined),
);
...
if (frontendWorkerCandidate) {
  ...
  if (missingIncludes.length)
    throw new Error(`Apps Script template is missing: ${missingIncludes.join(', ')}`);
  ...
  if (
    !index.includes("data-composite-requests-enabled=\"<?= compositeRequestsEnabled ? 'true' : 'false' ?>\"")
  ) {
  ...
  if (!appBody.includes('id="loading"') || !appBody.includes('id="primaryNav"'))
    throw new Error('AppBody.html is missing required interface markup.');
  if (!appStyles.includes('.app-shell') || !appStyles.includes('.loading-overlay'))
    throw new Error('AppStyles.html is missing required interface styles.');
  if (
    appBody.includes('<?') ||
    appBody.includes('?>') ||
    appStyles.includes('<?') ||
  ...
  const bodyTags = tokenizeHtml(appBody).filter(
    (token) => token.type === 'tag' && (token.name === 'script' || token.name === 'style'),
  );
  ...
  if (/<\/style/i.test(styleSource))
    throw new Error('AppStyles.html contains an unsafe closing-style sequence.');
  if (/<\/script/i.test(scriptSource))
    throw new Error('AppScript.html contains an unsafe closing-script sequence.');
  if (!scriptSource.includes('__HAU_RUNTIME_CONFIG__') || !scriptSource.includes('__HAU_APP_SCRIPT_LOADED__'))
    throw new Error('AppScript.html is missing Apps Script runtime markers.');
  if (!scriptSource.includes('DOMContentLoaded'))
    throw new Error('AppScript.html is missing the application bootstrap registration.');
  ...
  if (
    assembledAnalysis.bodyCount !== 1 ||
    assembledAnalysis.styleCount !== 1 ||
    assembledAnalysis.scriptCount !== 1
  ) {
    throw new Error(
      `Assembled Apps Script document has invalid executable element counts: ${JSON.stringify({ body: assembledAnalysis.bodyCount, style: assembledAnalysis.styleCount, script: assembledAnalysis.scriptCount })}`,
    );
  }
  if (assembledAnalysis.suspiciousVisibleJavaScriptTokenCount !== 0)
    throw new Error('Assembled Apps Script document exposes JavaScript as visible body text.');
  ...
  if (
    tokenizeHtml(assembled)
      ...
      ?.attributes.get('data-bootstrap-contract-version') !== '2'
  ) {
  ...
  if (
    tokenizeHtml(assembled)
      .find((token) => token.type === 'tag' && !token.closing && token.name === 'body')
      ?.attributes.get('data-composite-requests-enabled') !== 'false'
  ) {
  ...
  if (
    ...
    throw new Error(
      'Assembled Apps Script document did not default Venue and Equipment requests to disabled.',
    );
  }
  if (!diagnosticShell.includes('api_htmlDiagnosticPing') || !diagnosticShell.includes('data-inline-script'))
    throw new Error('DiagnosticShell.html is missing its isolated client/server checks.');
