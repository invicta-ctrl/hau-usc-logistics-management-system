...
EXTERNAL_ACTIONS: CANDIDATE_FREEZE=1;WORKFLOW_DISPATCH=1;PLAYGROUND_WORKER_DEPLOYMENT=1;PLAYGROUND_RESET=1;PROVIDER_EMAIL_DELIVERY=0;PLAYGROUND_D1_MIGRATION=0;PRODUCTION_MUTATION=0. All external identifiers, URLs, credentials, resource IDs, bookmarks, recipients, and private values remain redacted.
...
RECOVERY_RECEIPT: PLAYGROUND_D1_TIME_TRAVEL_BOOKMARK_PRESENT=TRUE;WORKER_ROLLBACK_HISTORY_PRESENT=TRUE;R2_CONFIG_IDENTITY=VERIFIED_REDACTED;D1_R2_PRODUCTION_CROSSOVER=FALSE;OWNER_AUTHORIZED_PRE_RESET_RECEIPT=CAPTURED_PRIVATE;RESET_RECONCILIATION=CLEAN_SCHEMA30_0030_FK_R2_PASS
BLOCKER: NONE;EXACT_FROZEN_PRE_MIGRATION_PLAYGROUND_DEPLOYMENT_VERIFIED;OWNER_AUTHORIZED_RESET_RECONCILED;0031_RECONCILIATION_IS_NEXT
...
RESUME_COMMANDS: Re-handshake the exact f8 frozen candidate and current reset-reconciled isolated Playground runtime/bindings; verify recovery has not changed; use only the accepted private manifest/config and remote D1 operator mechanism to list pending migrations, apply 0031 once, then reconcile without printing private values.
