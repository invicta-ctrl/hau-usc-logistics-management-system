...
- The next bounded slice is Materials & Documentation and is authorized but not started. Do not ingest `MATERIALS.html` until the Inventory & Pantry checkpoint is pushed and remote CI is verified.

Smallest safe next action:
- Push and verify the Inventory & Pantry checkpoint, then begin only the Materials & Documentation role checkpoint from `MATERIALS.html`.
