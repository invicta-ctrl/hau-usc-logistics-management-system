...
- The next bounded slice is Inventory & Pantry and is authorized but not started. Do not ingest `INVENTORY.html` until the Food checkpoint is pushed and remote CI is verified.

Smallest safe next action:
- Push and verify the Food checkpoint, then begin only the Inventory & Pantry role checkpoint from `INVENTORY.html`.
