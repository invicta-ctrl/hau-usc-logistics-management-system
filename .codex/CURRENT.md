...
STATUS: ACTIVE - OWNER-APPROVED ISOLATED STAGING IDENTITY ACCEPTANCE AND CLOSEOUT
BRANCH: GIT_BRANCH
HEAD: GIT_HEAD
...
Canonical migrations 0001-0030 are applied only to the new D1. Generation 3 contains 11 visibly synthetic actors (10 active, 1 disabled), 36 active inventory items, 3 requests, 3 deliverables, 2 lending tickets, and 1 event. Generation 2 is disabled/archived with 34 append-only ledger reversals. Current balances have zero negatives. A fresh export restores at schema 30 with all 30 migrations, integrity `ok`, and zero foreign-key violations.
...
Email containment has one exact private allowlist entry. A non-allowlisted same-domain probe produced the generic public response, created zero challenges, and made zero provider calls. The approved allowlisted recipient previously produced no challenge because protected identity eligibility rejected it before provider contact. The accepted owner amendment now authorizes a private staging-only exact-recipient fixture while preserving all production and protected-roster rules.
