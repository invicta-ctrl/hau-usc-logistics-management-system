...
RISK: MEDIUM
SCOPE: Implement the R3-A1 traceability items in priority order against `src/frontend/`; run the plain local Vite preview; perform local browser acceptance at 320/390/768/1024/1440; compare against the Figma Design current-authority references named in the handoff; keep the public Request Center reachable with no staff login and Staff Sign In separate; refresh the repository Make mirror at `output/design/figma-make-source/` from the saved Figma Make v40 source, reading each of the eight changed files verbatim from the provider rather than assuming the repository `src/frontend/` copy is identical, and record bytes and sha256 per file in the source and baseline registers.
