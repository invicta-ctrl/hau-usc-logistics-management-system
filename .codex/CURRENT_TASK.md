...
COMPLETED_SO_FAR: P00–P13 are complete at their assigned gates. P12 proved two live guarded reset cycles and left the isolated Playground at generation 6 CLEAN with zero sessions/transient rows. P13 added the Playground-only Administration reset center, safe baseline/generation/receipt projection, exact confirmation, `system.admin` + CSRF + server runtime enforcement, lock-conflict rejection, verified-final receipt publication, and fail-closed Production 404s. Its local candidate passes 165 files / 1222 tests and the 1679-module frontend build; exact deployed UI/reset E2E remains assigned to P29/P31.

NEXT_EXACT_ACTION: Begin P14 Profile and personalization. Reconcile existing profile/auth/preference/R2 contracts and implement only accepted missing behavior, preserving institution-controlled fields, audit/session invariants, and reset-baseline coverage.
