# Current Bounded Task — FI-LIVE-PREVIEW-01 Documentation Amendment

INTENT: DOCUMENTATION_WORKFLOW_AMENDMENT
MODE: REHYDRATE_PERSIST_VERIFY_HANDOFF
OBJECTIVE: Persist FI-LIVE-PREVIEW-01 and its token-efficient, loopback-only local-preview workflow for FI-01 through FI-12 without starting FI-02 or a preview.
TARGET: frontend-design-integration at `04f82930aca4eded8118ecea2560722d2f0b6bd6` / tree `0c0df80bd428c7693c3cdf3a8a7a0e597140cc30`.
CURRENT_POINTER: .codex/CURRENT.md
CURRENT_HANDOFF: .codex/CURRENT_HANDOFF.md
ACCEPTED_SPEC: .codex/specs/active/frontend-integration-fi01-shared-design-foundation.md
AUTHORITY: Earl FI-LIVE-PREVIEW-01 owner amendment -> project governance/current chain -> existing FI-01 accepted specification/integration packet -> existing guarded local-preview command.
REQUIRED_MODEL: gpt-5.6-terra sole branch writer, recorded as TERRA_MAX:/root/fi01_integration_writer.
ACTIVE_WRITER: TERRA_MAX:/root/fi01_integration_writer
RISK: LOW;DOCUMENTATION_ONLY
SCOPE: Accepted amendment record, integration plan/handoff/start-here/continuation discoverability, the eleven required current-handoff preview fields, current-chain linkage, focused documentation validation, normal commit/push/readback.
OUT_OF_SCOPE: FI-02 implementation/spec adoption; starting a preview/browser; private manifest resolution or disclosure; runtime/package/script/backend/API/auth/data/schema/migration/provider/Figma/Playground/Production changes; build/lint/E2E/deployment; rebase/reset/force/history rewrite or main merge.
VERIFICATION: Complete logical diff, targeted Prettier, git diff --check, check:agents, handoff:verify, check:continuation, exact-link/reference checks, secret/private-data scan, no port-4173 listener at closeout, normal push/readback parity.
STOP_CONDITIONS: Branch/upstream drift, dirty/conflicting writer, missing authority, runtime/private/provider access need, Production crossover, validator failure, private information exposure, or any non-documentation scope expansion.
WRITER_LOCK: HELD
LOCK_STATUS: HELD
HANDOFF_STATUS: FI_LIVE_PREVIEW_01_AMENDMENT_IN_PROGRESS
GIT_UPSTREAM: origin/frontend-design-integration@GIT_HEAD;PUSH_PARITY_VERIFIED_BEFORE_AMENDMENT
PRE_FI01_ROLLBACK_SHA: eacdfcc951c687cfca5731ede245130266b1c3da
PRE_FI01_ROLLBACK_TREE: 30b2ae1d15731d42fa668f48fe6a0064869ff655
ORIGIN_MAIN_SHA: 86553349f5c2ebefaa637c30828c560a301f99ba
STATUS: FI_LIVE_PREVIEW_01_AMENDMENT_IN_PROGRESS
ACCEPTED_AMENDMENT: .codex/specs/active/frontend-integration-live-local-preview-amendment.md;FI-LIVE-PREVIEW-01
NEXT_EXACT_ACTION: FI_LIVE_PREVIEW_01_DOCUMENTATION_CLOSEOUT

## Delivered and verified

### FI-LIVE-PREVIEW-01 amendment work in progress

- Persist the exact accepted amendment and link it through the current chain and
  FI packet; no local preview, private manifest lookup, browser opening, or
  provider action is authorized for this closed FI-01 session.
- The existing guarded command is documentation-only evidence: it requires an
  absolute private manifest, binds `127.0.0.1:4173`, and verifies the isolated
  Playground origin before launching Vite. The manifest path and identity are
  deliberately not read or recorded.

- `tokens.css` publishes the sole active D41 G1/G2/G3/G4 ladder, with tiered fill, saturation, border, shadow, and solid fallback values in both themes.
- The public bar, theme-toggle track, and back control consume G3, G1, and G2 respectively; their blur recipes contain no literal blur/saturation/fill/border/shadow values.
- The responsive route values are published from `tokens.css` to direct frame app surfaces. `v4.css` no longer contains an active container-query `:root`.
- FI-01 packet/receipt records D04 PASS, D02 PASS, D08 OPEN_FOR_FI02, and the next-slice/runtime-authority handoff.
- Repair commit `4c04105b85f47f1b1a40e66266f27484942b53a2` is normally pushed and read back at 0 ahead / 0 behind.

## Verified facts a future session must not re-derive

- FI-00 remains the frozen functional baseline; FI-01 changes shared CSS only, not backend/API/auth/data/migrations/provider state.
- `:root:not(*)` and `[data-theme]:not(*)` in retained legacy sheets are inert preservation evidence, not active token authority.
- Full `npm run lint` has the same pre-existing unrelated browser-global configuration failure in `prototypes/public-portals-r3/app.js`; do not change that prototype for FI-01. Focused `npx eslint src/v5` is the scoped check.

## Do not repeat before FI-02

- Do not reopen D-04 or D-02, re-audit Figma, add esbuild, or begin FI-02.
- Do not hand edit `dist/index.html` or `HAU-USC_Logistics-Prototype-Shareable.html`.
- Do not merge, rebase, reset, force-push, tag, deploy, migrate, or touch a provider without separately accepted authority.
