...
COMPLETED_SO_FAR: P00–P06 are complete. P07 preserved the passing but operationally incomplete privacy-filtered v1 baseline, derived and locally verified staging-safe v2 coverage without a new Production read, and published the pre-apply checkpoint. Reset attempt A failed without a success report. Read-only recovery proved D1 unchanged at generation 3 and R2 at the accepted sealed working-baseline state, with the temporary inspection Worker removed and Production mutation zero. The root cause is the missing Wrangler `--json` non-interactive restore mode; reset and installer rollback are repaired and regression-gated. No v2 overlay install has been attempted.

NEXT_EXACT_ACTION: Commit and push the P07 reset recovery checkpoint and restore repair, reverify the reconciled generation-3 state and a fresh report target, then execute corrected reset attempt B once. Reconcile any ambiguity before further action.
