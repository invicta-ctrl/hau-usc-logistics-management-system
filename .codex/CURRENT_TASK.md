# Current Task — V0.8.3 Isolated Playground Migration 0032 and Reconciliation

INTENT: ISOLATED_PLAYGROUND_MIGRATION
MODE: EXECUTE
OBJECTIVE: Preserve the frozen v0.8.3 candidate and its verified isolated Playground deployment, retain the reconciled CLEAN schema31/0031 working state, then apply only migration 0032 to the isolated Playground D1 and reconcile it before final automated acceptance.
TARGET: Frozen candidate f8e63372bc8afcb6d092970b7f9fc9ee72fd3580, tree 5788251d483f23ec5e19048e1a946b3a00450436, deployed to the isolated Playground at reconciled schema 31 / migration 0031.
...
SCOPE: A7-R2 Section 18 only: isolated Playground 0031 is complete and terminally reconciled; recheck redacted recovery, exact f8 runtime/binding identity, and the remote 0031 ledger immediately before mutation; apply only 0032 to the isolated Playground D1; then prove its ledger, schema, foreign keys, activity-history DDL/triggers, append-only invariants, and Production non-crossover. Record sanitized durable evidence before final automated acceptance.
...
MIGRATION_DECISIONS: SCHEMA_TARGET=32;0031=APPLIED_ONCE_AND_RECONCILED_ON_ISOLATED_PLAYGROUND_SCHEMA31;0032=SOURCE_PRESENT_AND_REQUIRED_BECAUSE_ACCEPTED_ID_G_INCLUDES_IT;ORDER=0031_THEN_0032;PLAYGROUND_RESET=PASS;PLAYGROUND_0031=PASS;PLAYGROUND_0032=AUTHORIZED_PENDING;PRODUCTION_APPLICATION=PROHIBITED
...
VERIFICATION: Reuse accepted Node 22.23.2 product evidence because f8 product blobs are unchanged. The reset and 0031 have terminally proven CLEAN schema31/0031, foreign keys, reset-probe absence, exact f8 runtime/bindings, canonical-identity invariants, recovery, and Production non-crossover. Before and after the one remote 0032 action, run only redacted recovery/identity/ledger/schema/foreign-key/activity-history-invariant/reconciliation checks plus governance, handoff, continuity, scope, and diff checks; do not rerun broad suites.
STOP_CONDITIONS: Conflicting candidate, writer, account, binding, or D1 identity; missing reversible recovery evidence; any Production crossover; migration already applied with inconsistent ledger/schema; P0/P1 security/privacy/data-integrity regression; private-data exposure; unexpected source/config mutation; failed 0032 reconciliation; or any action beyond isolated Playground 0032.
