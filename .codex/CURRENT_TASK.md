...
COMPLETED_SO_FAR: P00–P06 are complete. P07 preserved the passing but operationally incomplete privacy-filtered v1 baseline, derived deterministic staging-safe v2 coverage without a new Production read, and verified the v2 database and regenerated additive overlay locally. Schema 32/migration 0032, integrity, foreign keys, inventory reconciliation, R2 parity, role capabilities, and all required operational-domain coverage passed. Private-path-gated live install tooling now requires fixed identity, a clean reset, a reversible pre-apply bookmark, postflight coverage, exported reconciliation, a distinct v2 clean bookmark, and verified rollback. No P07 live reset/provider/database/R2 mutation has occurred.

NEXT_EXACT_ACTION: Commit and push the P07 pre-apply checkpoint, then reset the fixed Playground exactly once from the preserved v1 manifest and install the verified additive v2 overlay under the recorded bookmark, identity, privacy, integrity, reconciliation, and no-repeat gates.
