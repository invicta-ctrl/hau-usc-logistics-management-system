...
`REPOSITORY_CANDIDATE_READY - PREPRODUCTION_BLOCKED_PRIVATE_PROVIDER_CONFIGURATION`

The v0.7.2 product, schema, generated artifacts, and local verification are
complete at the release-branch working tree prepared for candidate freeze. The
exact product SHA is recorded by the freeze commit that adds this handoff; the
release-record follow-up must name that SHA without changing product source.
...
- Final `npm run check`: 109 files / 726 tests passed; lint, two deterministic
