...
| Figma Make | EXISTS and is on the canonical theme at **Version 39**, file `rP9W9MQlZkyQrUx38TVsFS`. The former "NOT APPLICABLE / no Make project exists" record was **stale and wrong** — it described a toolchain limitation, not the file. v37 adopted the canonical theme and the four route files, v38 pinned the landing atrium, v39 scoped the atrium palette pin (MK-06). Make is no longer the surface carrying its own palette. |
