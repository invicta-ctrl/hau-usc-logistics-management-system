 ...
 *   staff and officers only, session required. It lives in
 ...
 *
 * SUPERSEDED
 *   The previous revision described the logistics Request Center as public and
 *   carried "PUBLIC REQUEST · NO SIGN-IN" copy. That matched the then-current
 *   authority. The owner corrected the product policy in R3-A1-A2; that reading
 *   is historical, not current. DESIGN.md D24.0 — OWNER-LOCKED no-login public
 *   LENDING — is untouched and remains the authority for this file.
 *
 * PROTOTYPE
 *   This is the Figma Make prototype. It simulates outcomes and stores nothing.
 *   Interaction evidence only — never a backend or a runtime contract.
 */
...
export default function PublicFlows({
  ...
  onBack: () => void;
  dark: boolean;
  onNavigat: (route: "request-center" | "lending") => void;
}) {
  const initial: View =
    route === "tracking" ? "Track lending" : "Lending Center";
      ...
      {view === "Lending Center" && (
        <>
          ...
          {!lendReceipt ? (
            <form className="panel glass" onSubmit={(e) => {
              ...
              {borrowerType && (
                <>
                  ...
                  <button className="primary">Submit borrowing request for review</button>
            ...
            <Receipt kind="Borrowing request" id={lendReceipt.id} onClose={() => setLendReceipt(null)} />
          )}
        </>
      )}
