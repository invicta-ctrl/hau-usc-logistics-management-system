...
import { MOBILE_DOCK } from "./navConfig";
...
export function AuthenticatedShell({
  ...
  return (
    <div className="min-h-screen flex" style={{ background: dark ? "#1c1917" : "#fffdf8" }}>
      <AuthShellSidebar route={route} navigate={navigate} session={session} onSignOut={onSignOut} />

      <div className="flex flex-col flex-1 min-w-0 lg:ml-[168px] min-h-screen">
        <AuthShellTopbar
          route={route}
          navigate={navigate}
          session={session}
        ...
        >
          {MOBILE_DOCK.map((item) => (
            ...
            >
              ...
              />
              <span style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", fontSize: 10, color: route === item.route ? "#e8b93c" : "rgba(250,238,203,0.5)", letterSpacing: -0.1 }}>
                {item.label}
              </span>
            </button>
          ))}
