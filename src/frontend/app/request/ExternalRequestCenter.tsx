...
export function ExternalRequestCenter({
  ...
  useEffect(() => {
    ...
    setState("loading");
    setAlert("");
    void frontendBackend.requesterPortal(controller.signal).then((next) => {
      setPortal(next);
      setState("ready");
    }).catch((error: unknown) => {
      ...
      setState(code === "REQUESTER_PORTAL_REQUIRED" || code === "CAPABILITY_REQUIRED" ? "portal-unavailable" : "error");
      setAlert(error instanceof FrontendApiError ? error.message : "The request service is temporarily unavailable.");
    });
  ...
  const openRequests = useMemo(
    () => (portal?.requests ?? []).filter((request) => !["CANCELLED", "REJECTED", "COMPLETED"].includes(request.status)),
    [portal],
  );
  ...
  async function submit() {
    if (!canSubmit) {
      setAlert("Complete the event, purpose, at least one requested item, and every acknowledgment before submitting.");
      return;
    }
    setAlert("");
    setBusy(true);
      ...
      <main id="main-content" className="flex-1 w-full max-w-[1120px] mx-auto px-5 md:px-8 py-8 flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-2">
          ...
          <p style={{ fontSize: 13, color: c.muted, maxWidth: 640, lineHeight: "20px" }}>
            Submit USC operational needs — inventory and pantry restocking, office supplies, event materials and food,
            venue and activity support. Signed in as <strong style={{ color: c.text }}>{session.displayName}</strong>
            {portal?.profile.displayName ? <> for <strong style={{ color: c.text }}>{portal.profile.displayName}</strong></> : null}.
          </p>
        </section>

        {state === "loading" && (
          <section className="rounded-[14px] px-5 py-6" style={{ background: c.m1, border: `1px solid ${c.border}` }} aria-busy="true">
            <p style={{ fontSize: 13, color: c.muted }}>Loading your requests…</p>
          </section>
        )}

        {state === "portal-unavailable" && (
          <section role="alert" className="rounded-[14px] px-5 py-6 flex flex-col gap-3" style={{ background: c.m1, border: `1px solid ${c.border}` }}>
            <strong style={{ fontSize: 14, color: c.text }}>The requester portal is not available for this account</strong>
            <p style={{ fontSize: 13, color: c.muted, lineHeight: "20px" }}>
              Your account is signed in, but the request service does not currently accept submissions from it.
              This is a known limitation on the service side, not a problem with your sign-in.
            </p>
            ...
            <div className="flex flex-wrap gap-3">
              {session.internalOperator && (
                <button type="button" onClick={onOpenLogisticsHub} style={primaryButton}>Open Logistics Hub</button>
              )}
              <button type="button" onClick={onHome} style={quietButton}>Home</button>
            </div>
          </section>
        )}
        ...
        {state === "error" && (
          <section role="alert" className="rounded-[14px] px-5 py-6 flex flex-col gap-3" style={{ background: c.m1, border: `1px solid ${c.border}` }}>
            <strong style={{ fontSize: 14, color: c.text }}>The request service is temporarily unavailable</strong>
            <p style={{ fontSize: 13, color: c.muted }}>{alert}</p>
            <div className="flex gap-3">
              <button type="button" onClick={() => setReload((n) => n + 1)} style={primaryButton}>Try again</button>
              <button type="button" onClick={onHome} style={quietButton}>Home</button>
            </div>
          </section>
        )}
        ...
        {receipt && (
          <section role="status" className="rounded-[14px] px-5 py-6 flex flex-col gap-2" style={{ background: c.m1, border: "1px solid #d1b478" }}>
            <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, letterSpacing: "1px", textTransform: "uppercase", color: dark ? "#e8b93c" : "#7d5518" }}>
              Request submitted
            </p>
            <strong style={{ fontSize: 16, color: c.text }}>{receipt.id}</strong>
            <p style={{ fontSize: 13, color: c.muted }}>
              {receipt.event}{receipt.subEvent ? ` · ${receipt.subEvent}` : ""} · {pretty(receipt.status)}.
              It appears in your requests below and is visible to the Department of Logistics for review.
            </p>
          </section>
        )}
          ...
          <>
            ...
            <section className="flex flex-col gap-3" aria-label="Your requests">
              ...
              {portal.requests.length === 0 ? (
                <div className="rounded-[14px] px-5 py-6" style={{ background: c.m1, border: `1px solid ${c.border}` }}>
                    ...
                    <li key={request.id} className="rounded-[14px] px-5 py-4 flex flex-col gap-2" style={{ background: c.m1, border: `1px solid ${c.border}` }}>
                      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        ...
                        {request.requestType === "ADDITIONAL" && (
                          <span style={{ fontSize: 11, color: c.muted }}>Additional to {request.parentRequestId}</span>
                        )}
                      </div>
                      <p style={{ fontSize: 13, color: c.text }}>{request.event}{request.subEvent ? ` · ${request.subEvent}` : ""}</p>
                      <p style={{ fontSize: 12, color: c.muted, lineHeight: "18px" }}>{request.purpose}</p>
                      <p style={{ fontSize: 11, color: c.muted }}>
                        {request.lines.length} line{request.lines.length === 1 ? "" : "s"} · updated {request.updatedAt || "not reported"}
                      </p>
                    </li>
                  ))}
                </ul>
        ...
        {state === "ready" && portal && composing && (
          <section className="flex flex-col gap-5" aria-label="New request">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="flex flex-col gap-1.5" htmlFor={`${formId}-type`} style={{ fontSize: 13, color: c.text }}>
                Request type
                <select id={`${formId}-type`} value={requestType} onChange={(e) => setRequestType(e.target.value as "NEW" | "ADDITIONAL")} style={field}>
              ...
              {requestType === "ADDITIONAL" && (
                <label className="flex flex-col gap-1.5" htmlFor={`${formId}-parent`} style={{ fontSize: 13, color: c.text }}>
                  Parent request
                  <select id={`${formId}-parent`} value={parentRequestId} onChange={(e) => setParentRequestId(e.target.value)} style={field}>
                    <option value="">Select a request</option>
                    {openRequests.map((request) => (
                      <option key={request.id} value={request.id}>{request.id} — {request.event}</option>
                    ))}
                  </select>
                </label>
              )}

              <label className="flex flex-col gap-1.5" htmlFor={`${formId}-series`} style={{ fontSize: 13, color: c.text }}>
                Event series
                <select
                  id={`${formId}-series`}
                  value={eventSeriesId}
                  onChange={(e) => { setEventSeriesId(e.target.value); setEventId(""); }}
                  style={field}
                >
              ...
              <label className="flex flex-col gap-1.5" htmlFor={`${formId}-event`} style={{ fontSize: 13, color: c.text }}>
                Event or sub-event
                <select id={`${formId}-event`} value={eventId} onChange={(e) => setEventId(e.target.value)} style={field} disabled={!eventSeriesId}>
                  <option value="">Select an event</option>
                  {eventsForSeries.map((event) => (
                    <option key={event.id} value={event.id}>{event.name}{event.venue ? ` — ${event.venue}` : ""}</option>
                  ))}
                </select>
              </label>
            </div>

            <label className="flex flex-col gap-1.5" htmlFor={`${formId}-purpose`} style={{ fontSize: 13, color: c.text }}>
              Purpose
              <textarea
                id={`${formId}-purpose`}
                value={purpose}
                ...
                return (
                  <div key={index} className="rounded-[14px] px-4 py-4 flex flex-col gap-3" style={{ background: c.m1, border: `1px solid ${c.border}` }}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <label className="flex flex-col gap-1.5" htmlFor={`${formId}-cat-${index}`} style={{ fontSize: 12, color: c.text }}>
                        Category
                        <select
                          id={`${formId}-cat-${index}`}
                          value={line.category}
                          onChange={(e) => updateLine(index, { category: e.target.value, description: "", custom: false })}
                          style={field}
                        >
                      ...
                      <label className="flex flex-col gap-1.5" htmlFor={`${formId}-item-${index}`} style={{ fontSize: 12, color: c.text }}>
                        Item
                        {choices.length > 0 && !line.custom ? (
                          <select
                            ...
                            onChange={(e) => {
                              if (e.target.value === "__custom") updateLine(index, { custom: true, description: "" });
                      ...
                      <label className="flex flex-col gap-1.5" htmlFor={`${formId}-qty-${index}`} style={{ fontSize: 12, color: c.text }}>
                        Quantity
                        <input
                          id={`${formId}-qty-${index}`}
                          type="number"
                      ...
                      <label className="flex flex-col gap-1.5" htmlFor={`${formId}-unit-${index}`} style={{ fontSize: 12, color: c.text }}>
                        Unit
                        <select id={`${formId}-unit-${index}`} value={line.unit} onChange={(e) => updateLine(index, { unit: e.target.value })} style={field}>
                          {units.map((unit) => <option key={unit} value={unit}>{unit}</option>)}
                        </select>
                      </label>
                    </div>

                    <label className="flex flex-col gap-1.5" htmlFor={`${formId}-spec-${index}`} style={{ fontSize: 12, color: c.text }}>
            ...
            <fieldset className="flex flex-col gap-3">
              <legend style={{ fontSize: 13, fontWeight: 600, color: c.text, marginBottom: 8 }}>Acknowledgments</legend>
              {ACKS.map((ack) => (
                <label key={ack.name} className="flex gap-3 items-start rounded-[10px] px-4 py-3" style={{ background: c.m1, border: `1px solid ${c.border}` }}>
                  <input
                    type="checkbox"
            ...
            <p style={{ fontSize: 12, color: c.muted, lineHeight: "18px" }}>
              Submitting does not reserve anything and deducts no stock. The Department of Logistics reviews the
              request first; reservation and release are recorded separately.
            </p>

            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => void submit()} disabled={!canSubmit} style={{ ...primaryButton, opacity: canSubmit ? 1 : 0.55 }}>
                {busy ? "Submitting…" : "Submit request"}
              </button>
              <button type="button" onClick={() => { setComposing(false); setAlert(""); }} style={quietButton}>
                Cancel
              </button>
            </div>
          </section>
