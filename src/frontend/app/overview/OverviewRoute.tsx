...
export function OverviewRoute({ session, dark }: { session: Session; dark: boolean }) {
    ...
    <div className="command-table-page">
      <header className="command-table-header">
        ...
        <p className="command-summary">
          <strong>14</strong> open requests · <strong>9</strong> loans out · <strong>6</strong> awaiting release · <strong>2</strong> below threshold
        </p>
        <p className="command-record">Authorized preview · {session.displayName} · local state only</p>
      </header>
      ...
      <div className="command-support-grid">
        <section className="command-pulse" aria-labelledby="command-pulse-heading">
          <div className="command-section-head"><h2 id="command-pulse-heading">Operational pulse</h2><span>Latest recorded activity</span></div>
          {loading ? (
        ...
        <section className="command-topology" aria-labelledby="command-topology-heading">
          <div className="command-section-head"><h2 id="command-topology-heading">Ledger topology</h2><span>Request to return</span></div>
          {loading ? (
            <ul className="command-topology-list" aria-label="Loading ledger topology" aria-busy="true">
              {[1, 2, 3, 4, 5, 6].map((item) => <li key={item}><SkeletonBlock h={58} dark={dark} /></li>)}
            </ul>
          ) : (
            <ol className="command-topology-list">
              {TOPOLOGY.map((node) => (
                <li key={node.key} className={`command-topology-node${topologyKeys[activePath] === node.key ? " is-active" : ""}`}>
                  <span>{node.label}</span><strong>{node.val}</strong>
                </li>
              ))}
            </ol>
          )}
      ...
      <section className="command-reconciliation" aria-labelledby="command-recon-heading">
        <div className="command-section-head"><h2 id="command-recon-heading">Ledger reconciliation</h2><span>Projection against ledger</span></div>
        {loading ? (
          <ul className="command-recon-list" aria-label="Loading reconciliation" aria-busy="true">
            {[1, 2, 3].map((item) => <li key={item}><SkeletonBlock h={56} dark={dark} /></li>)}
          </ul>
        ) : (
          <ul className="command-recon-list">
            {RECON_ROWS.map((row) => (
              <li className="command-recon-row" key={row.label}>
                ...
                <div className="command-recon-values"><span>Projection {row.projection}</span><span>Ledger {row.ledger}</span></div>
