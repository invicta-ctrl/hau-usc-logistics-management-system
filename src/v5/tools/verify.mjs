...
for (const width of WIDTHS) {
  ...
  /* v3 has staged entrance motion. Mid-animation, transformed elements report
  ...
  const findings = await page.evaluate(() => {
    ...
    for (const id of ids) {
      ...
      for (const st of states) {
        ...
        // Primary controls must meet the v3 44px interaction floor. Inline
        // prose links and native check/radio controls use the WCAG spacing
        // exception and are intentionally outside this geometry check.
        document
          .querySelectorAll(
            '.frame button, .frame select, .frame input:not([type="checkbox"]):not([type="radio"]):not([readonly]), .frame textarea, .frame .search, .frame .back-control, .frame .portal-card',
          )
          .forEach((el) => {
  ...
  if ([320, 768, 1440].includes(width)) {
    for (const [id, st] of SHOTS) {
      ...
      /* View-transition pseudo-elements are outside the universal freeze rule.
         Wait past the bounded surface token so the capture records the final
         route rather than an old/new snapshot gap. */
...

{
  ...
  const before = await page.evaluate(() => {
    ...
    if (!btn) return null;
    btn.focus();
    return document.activeElement.dataset.act;
  });
  if (before) await page.click('[data-act="confirm-accept"]');
  await page.waitForSelector('[role="dialog"][aria-modal="true"]');
  const dialog = await page.evaluate((trigger) => ({
    before: trigger,
    open: !!document.querySelector('[role="dialog"][aria-modal="true"]'),
    focusMovedIn: !!document.querySelector('[data-overlay-root]')?.contains(document.activeElement),
    focusedAction: document.activeElement?.dataset?.act ?? null,
  }), before);
