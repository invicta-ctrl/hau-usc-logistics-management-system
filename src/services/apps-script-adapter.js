  ...
  _call(method, command = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      runner.withSuccessHandler((result) => {
        ...
        let normalized;
        try { normalized = result && typeof result === 'object' ? JSON.parse(JSON.stringify(result)) : result; }
        catch { reject(new AppError('APPS_SCRIPT_FAILURE', 'Apps Script returned an unsupported response.', { retryable: true })); return; }
        if (!normalized?.ok) reject(new AppError(normalized?.code ?? 'SERVER_ERROR', normalized?.message ?? 'The operation failed.', { correlationId: normalized?.correlationId, retryable: Boolean(normalized?.retryable), details: normalized?.details }));
        else resolve(normalized);
      }).withFailureHandler((error) => { clearTimeout(timeout); reject(new AppError('APPS_SCRIPT_FAILURE', error?.message ?? 'Apps Script failed.', { retryable: true })); })[method](command);
    });
  }
}
