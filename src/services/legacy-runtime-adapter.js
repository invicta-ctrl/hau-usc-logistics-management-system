...
export function createLegacyRuntimeAdapter(mockServices) {
  ...
  return {
    ...
    async confirmReturn(ticketId, notes = '') { const result = await mutationRequests.run('lending-return', { ticketId, notes }, (command) => remote.confirmReturn(command)); return { id: result.ticketId, ...result }; },
