...
// The canonical request projection has no stored date range, so render the
// requested window only when the active contract actually supplies one.
function requestWindow(request) {
  if (request.dateStart && request.dateEnd) return `${request.dateStart} to ${request.dateEnd}`;
  return request.createdAt ? `Submitted ${String(request.createdAt).slice(0, 10)}` : 'Submission date not reported';
}
...
export function renderRequests(ctx) {
  ...
  ${requestOnly ? '' : `<section class="panel"><div class="panel-head"><div><h2>Review queue</h2><p>Accepting a request validates every line and commits all reservations together.</p></div><span class="pill">${internalQueue.length} requests</span></div><div class="list">${internalQueue.map((request) => `<article class="list-item"><span><div class="button-row">${statusChip(request.status)}<code>${request.id}</code></div><strong>${escapeHtml(request.purpose)}</strong><small>${escapeHtml(request.requesterName ?? 'Requester not reported')} · ${escapeHtml(requestWindow(request))}</small></span>${request.status === 'FOR_REVIEW' ? `<button class="primary" type="button" data-accept-request="${request.id}">Accept & route</button>` : ''}</article>`).join('')}</div></section>`}</div>`;
}
