...
const THEME_KEY = 'hau-usc-v3-theme';
const store = {
  get(key) {
    try {
...
function previewBar() {
  ...
  return `<div class="preview-bar">
    <div class="preview-bar__brand">
      ${icon('box', 'icon--sm')}
      <span>HAU-USC Logistics · Redesign preview v3</span>
...
function topbar() {
  ...
  return `<header class="topbar">
    <button class="menu-control" type="button" data-act="toggle-rail"
      aria-expanded="${state.drawer === 'open'}" aria-controls="primary-navigation"
      aria-label="${state.drawer === 'open' ? 'Close navigation' : 'Open navigation'}">
    ...
    body = `<div class="frame" data-viewport="${state.viewport}">
      <div class="shell" data-rail="${state.rail}" data-drawer="${state.drawer}">
        ...
        <section class="workspace">
          ${topbar()}
          <div class="route-progress" aria-hidden="true"><span></span></div>
