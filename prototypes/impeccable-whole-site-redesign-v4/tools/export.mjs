...
const css = sourceCss.replace(
  /url\(["']?\.\.\/assets\/fonts\/([^"')]+)["']?\)/g,
  (_match, file) => {
    const bytes = readFileSync(join(previewRoot, 'assets', 'fonts', file));
    const mime = file.endsWith('.woff2') ? 'font/woff2' : 'application/octet-stream';
...
<html lang="en">
  <head>
    ...
    <!--
      ...
      Regenerate: node prototypes/impeccable-whole-site-redesign-v4/tools/export.mjs
...
const outFile = join(outDir, 'HAU_USC_Logistics_Impeccable_Whole_Site_Redesign_Preview_v4.html');
